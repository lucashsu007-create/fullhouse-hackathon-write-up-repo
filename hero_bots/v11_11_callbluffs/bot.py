...
def default_stats():
    return {
        ...
        "faced_bet_flop": 0, "fold_vs_bet_flop": 0,
        "faced_bet_turn": 0, "fold_vs_bet_turn": 0,
        "faced_bet_river": 0, "fold_vs_bet_river": 0,
...
def update_stats(state):
    ...
    for rec in recon[already:]:
        ...
        if owed > 0:
            ...
            # Per-street fold-to-bet (lets the overfold read condition on street)
            _sk = {1: "flop", 2: "turn", 3: "river"}.get(street)
            if _sk:
                st["faced_bet_%s" % _sk] += 1
                if act == "fold":
                    st["fold_vs_bet_%s" % _sk] += 1
...
def _safetag(state):
    ...
    eq = _hero_equity(hole, board, state, n_opp, rng)
    if eq is not None:
        return _postflop_with_exploit(state, eq, pos, can_check, board, rng, hole)
...
# ===========================================================================
# v11 READ-DRIVEN EXPLOIT (inlined from v11_exploit.py v4: reads + policies)
# Single-file for submission. Pure-stdlib. exploit_plan() is the pinned
# deterministic core; _postflop_with_exploit() below is the splice.
# ===========================================================================
...
def _betainc(a, b, x):
    ...
    front = math.exp(math.log(x) * a + math.log(1.0 - x) * b - lb)
    if x < (a + 1.0) / (a + b + 2.0):
        return front * _betacf(a, b, x) / a
    return 1.0 - front * _betacf(b, a, 1.0 - x) / b

def _beta_sf(t, a, b):          # P(p > t)
    return 1.0 - _betainc(a, b, t)

def breakeven_fold(bet_frac):
    """Pure-bluff break-even fold freq for a bet of size s*pot: s/(1+s)."""
    return bet_frac / (1.0 + bet_frac)

def _posterior(folds, faced, prior_m=0.5, prior_k=6.0):
    a = prior_m * prior_k + folds
    b = (1.0 - prior_m) * prior_k + (faced - folds)
    return a, b

def _street_counts(stats, street):
    fk = "fold_vs_bet_%s" % street
    nk = "faced_bet_%s" % street
    if nk in stats:
        return stats.get(fk, 0), stats.get(nk, 0)
    return stats.get("fold_vs_bet", 0), stats.get("faced_bet", 0)

def overfold_read(stats, street, bet_frac, conf=0.90, min_n=20):
    """Does this villain fold to a `bet_frac`-pot bet on `street` above its
    break-even? Returns (fires, edge>=0, p_excess). The reusable bluff read —
    every barrel/bluff policy calls this with its own street and size."""
    if not stats:
        return False, 0.0, 0.0
    folds, faced = _street_counts(stats, street)
    if faced < min_n or not (0 <= folds <= faced):     # garbage -> no read
        return False, 0.0, 0.0
    f_star = breakeven_fold(bet_frac)
    a, b = _posterior(folds, faced)
    p_excess = _beta_sf(f_star, a, b)
    fires = p_excess >= conf
    edge = max(0.0, (a / (a + b)) - f_star)
    return fires, edge, p_excess

def station_read(stats, conf=0.90, min_n=20, call_floor=0.62):
    """Does this villain call too much (confidently fold below 1-call_floor)?
    Returns (fires, looseness>=0). The reusable value read."""
    if not stats:
        return False, 0.0
    faced = stats.get("faced_bet", 0)
    if faced < min_n:
        return False, 0.0
    folds = stats.get("fold_vs_bet", 0)
    if not (0 <= folds <= faced):                      # garbage -> no read
        return False, 0.0
    a, b = _posterior(folds, faced)
    fold_cap = 1.0 - call_floor
    p_low = _betainc(a, b, fold_cap)
    fires = p_low >= conf
    looseness = max(0.0, call_floor - (a / (a + b)))
    return fires, looseness


def bet_punisher_read(stats, raise_cap=0.15, prior_m=0.10, prior_k=8.0, min_n=20):
    """True if villain punishes BETS with raises often enough that bluffing into
    them is dangerous (check-raiser / trapper). Uses the raise_vs_bet counter,
    shrunk toward a low population raise rate so thin/unknown villains are NOT
    flagged. Trap-field defense: fold_vs_bet alone calls a check-raiser an
    'overfolder', but their elevated raise_vs_bet means our bet gets raised."""
    if not stats:
        return False
    faced = stats.get("faced_bet", 0)
    raises = stats.get("raise_vs_bet", 0)
    if faced < min_n or not (0 <= raises <= faced):
        return False
    a = prior_m * prior_k + raises
    b = (1.0 - prior_m) * prior_k + (faced - raises)
    return (a / (a + b)) > raise_cap


def _aggr_freq_post(stats, prior_m=0.25, prior_k=8.0):
    """Beta posterior on the villain's overall aggression frequency
    (raises + all-ins as a share of all actions). Returns (a, b, n) or None."""
    if not stats:
        return None
    acts = stats.get("actions", 0)
    aggr = stats.get("raises", 0) + stats.get("allins", 0)
    if acts <= 0 or not (0 <= aggr <= acts):
        return None
    a = prior_m * prior_k + aggr
    b = (1.0 - prior_m) * prior_k + (acts - aggr)
    return a, b, acts


def over_bluffer_read(stats, conf=0.80, min_n=40, aggr_floor=0.45):
    """Confident the villain is hyper-aggressive (bets/raises a large share of
    the time) -> betting range runs bluff-heavy -> hero can CALL lighter.
    P(aggr_rate > aggr_floor) >= conf. Stations/nits (low aggr) are excluded."""
    post = _aggr_freq_post(stats)
    if post is None:
        return False
    a, b, acts = post
    if acts < min_n:
        return False
    return (1.0 - _betainc(a, b, aggr_floor)) >= conf


def value_heavy_read(stats, raise_cap=0.18, min_n=20):
    """Confident the villain check-raises / raises our bets often -> when it puts
    in a raise it is value-weighted -> hero FOLDS marginal hands instead of
    paying off. This is bet_punisher on the defensive side; because it keys on
    raise_vs_bet, calling stations (low raise_vs_bet) are NOT flagged."""
    return bet_punisher_read(stats, raise_cap=raise_cap, min_n=min_n)


# ---- defensive (facing-a-bet) read-driven adjustment knobs ----
DEF_FOLD_VS_VALUE   = False     # fold marginal hands vs confirmed value-heavy / traps
DEF_CALL_VS_BLUFF   = True     # call marginal folds vs confirmed over-bluffers
DEF_FOLD_DELTA      = 0.05     # only flip CALLs whose eq is within this band ABOVE pot odds
DEF_CALL_DELTA      = 0.05     # only flip FOLDs whose eq is within this band BELOW pot odds
DEF_VALUE_RAISE_CAP = 0.18     # raise_vs_bet shrunk-mean above this => value-heavy
DEF_CALL_CONF       = 0.80
DEF_AGGR_FLOOR      = 0.45
DEF_MIN_ACTS        = 40
...
# ===========================================================================
# READS — handle. Bundles the estimators for one decision so a policy consumes
# `reads`, not raw `stats`. (Posterior caching could live here later if several
# policies query the same street in one decision; not needed yet.)
# ===========================================================================
class Reads:
    __slots__ = ("stats",)
    ...
    def overfold(self, street, bet_frac, conf=0.90, min_n=20):
        return overfold_read(self.stats, street, bet_frac, conf, min_n)

    def station(self, conf=0.90, min_n=20, call_floor=0.62):
        return station_read(self.stats, conf, min_n, call_floor)

    def bet_punisher(self, raise_cap=0.15, min_n=20):
        return bet_punisher_read(self.stats, raise_cap=raise_cap, min_n=min_n)


@dataclass(frozen=True, slots=True)
class Spot:
    """Decision context handed to every policy. Built once in exploit_plan()."""
    street: str
    can_check: bool
    equity: float
    pos: float
...
# ===========================================================================
# POLICIES — registry (was the "lever" registry; renamed for the split).
# ===========================================================================
# Sentinel: "this policy owns the spot but declines to act — stop the chain."
DECLINE = object()

Policy = Callable[[Reads, Spot, dict], object]
POLICIES: List[Tuple[str, Policy]] = []

# Per-policy config. Defaults equal v3's, so behaviour is unchanged; tune these
# from D1 data without touching code. min_lambda=0.0 makes the floor gate inert.
POLICY_CONFIG: Dict[str, dict] = {
    "thin_value_vs_station": {
        "enabled": True, "min_lambda": 0.0,
        "conf": 0.90, "min_n": 20, "call_floor": 0.62,
        "value_cut_base": 0.55, "value_size_base": 0.70,
    },
    "bluff_vs_overfolder": {
        "enabled": True, "min_lambda": 0.0,
        "conf": 0.90, "min_n": 20, "raise_cap": 0.15,
        "bluff_cut": 0.42, "bluff_size": 0.60,
    },
}
...
# ---- the two ported policies. Cheap, read-independent guards run first so the
#      posterior (betainc) is skipped whenever the spot can't produce an action.
@policy("thin_value_vs_station")
def _thin_value(reads: Reads, spot: Spot, cfg: dict):
    if not spot.can_check:
        return None                                    # PASS: not our spot
    st_fire, loose = reads.station(conf=cfg["conf"], min_n=cfg["min_n"],
                                   call_floor=cfg["call_floor"])
    if not st_fire:
        return None                                    # PASS: not a station
    value_cut = max(0.45, cfg["value_cut_base"] - 0.6 * loose)
    if spot.equity >= value_cut:
        size = min(0.9, cfg["value_size_base"] + 0.5 * loose)
        return {"action": "raise", "_kind": "thin_value",
                "_size": size, "_value_cut": value_cut}
    return DECLINE                                     # station owns spot; no bet


@policy("bluff_vs_overfolder")
def _bluff(reads: Reads, spot: Spot, cfg: dict):
    if not (spot.can_check and spot.equity < cfg["bluff_cut"]):
        return None
    if spot.street in ("flop", "turn") and not spot.has_draw:
        return None                                    # no naked multi-street barrel
    if spot.street == "river" and not spot.capped:
        return None
    of_fire, edge, _p = reads.overfold(spot.street, cfg["bluff_size"],
                                       conf=cfg["conf"], min_n=cfg["min_n"])
    if not of_fire:
        return None
    # trap-field defense: don't bluff a villain who punishes bets with raises
    # (check-raiser / trapper). fold_vs_bet flags them as overfolders, but their
    # elevated raise_vs_bet means our bet gets check-raised -> abort.
    if reads.bet_punisher(raise_cap=cfg["raise_cap"], min_n=cfg["min_n"]):
        return None
    depth = {"flop": 0.45, "turn": 0.70, "river": 1.0}.get(spot.street, 1.0)
    freq = max(0.25, min(0.75, 0.25 + 2.5 * edge)) * depth
    return {"action": "raise", "_kind": "bluff",
            "_size": cfg["bluff_size"], "_freq": freq, "_edge": edge}


def _run_policies(reads: Reads, spot: Spot, overrides=None, lam=1.0):
    """Chain-of-responsibility over registered policies. First CLAIM or DECLINE
    settles the spot; PASS (None) tries the next. DECLINE collapses to None on
    the way out, so exploit_plan()'s external contract is unchanged."""
    if spot.street == "preflop":
        return None
    overrides = overrides or {}
    for name, fn in POLICIES:
        cfg = dict(POLICY_CONFIG.get(name, {}))
        cfg.update(overrides.get(name, {}))
        if not cfg.get("enabled", True):
            continue
        if lam < cfg.get("min_lambda", 0.0):           # identity at min_lambda=0
            continue
        result = fn(reads, spot, cfg)
        if result is DECLINE:
            return None
        if result is not None:
            if RECORD_TELEMETRY and len(TELEMETRY) < _TELEMETRY_CAP:
                TELEMETRY.append({"policy": name, "street": spot.street,
                                  "lam": lam, "plan": dict(result)})
            return result
    return None


# ===========================================================================
# Public API — signatures + behaviour identical to v3.
# ===========================================================================
def exploit_plan(stats, *, street, can_check, equity, pos, heads_up, capped,
                 has_draw, base_value_cut=0.55, base_bluff_cut=0.42,
                 bluff_size=0.60, value_size=0.70, lam=1.0):
    """Returns a deviation plan dict, or None when no confident read applies
    (FLOOR GUARD). `lam` is optional (default 1.0) and leaves the default-off
    floor gate inert."""
    reads = Reads(stats)
    spot = Spot(street=street, can_check=can_check, equity=equity, pos=pos,
                heads_up=heads_up, capped=capped, has_draw=has_draw)
    overrides = {
        "thin_value_vs_station": {"value_cut_base": base_value_cut,
                                  "value_size_base": value_size},
        "bluff_vs_overfolder":   {"bluff_cut": base_bluff_cut,
                                  "bluff_size": bluff_size},
    }
    return _run_policies(reads, spot, overrides=overrides, lam=lam)


def _villain_is_capped(state, villain_seat):
    """River range likely has no nutted hands: villain never raised postflop
    this hand and was not the preflop aggressor. Necessary-not-sufficient (a
    slowplayed monster also looks capped); the 60%-pot sizing bounds the cost
    of a misclassification."""
    s = _villain_action_summary(state, villain_seat)
    if s.get("raised_postflop"):
        return False
    if s.get("raised_pre") or s.get("threebet_pre") or s.get("four_bet_pre"):
        return False
    return True


def _postflop_with_exploit(state, eq, pos, can_check, board, rng, hole):
    """Apply the read-driven exploit (exploit_plan) over the baseline postflop
    policy. HEADS-UP ONLY; multiway or no confident read -> EXACT baseline
    behaviour. The bluff frequency mix uses a salted, separate RNG so the
    baseline RNG stream is untouched whenever the exploit declines -- i.e. the
    bot deviates from v7_m2 ONLY on a real thin-value/bluff action."""
    villains = _live_villains(state)
    if len(villains) != 1:
        return _postflop_by_equity(state, eq, pos, can_check, board, rng)
    vs_seat, vs_bot_id = villains[0]
    v_stats = PLAYER_STATS.get(vs_bot_id)
    _, _fd, _oesd = _postflop_strength(hole, board)
    street = {3: "flop", 4: "turn", 5: "river"}.get(len(board), "flop")
    plan = exploit_plan(v_stats, street=street, can_check=can_check, equity=eq,
                        pos=pos, heads_up=True,
                        capped=_villain_is_capped(state, vs_seat),
                        has_draw=bool(_fd or _oesd))
    if plan is not None:
        if plan["_kind"] == "thin_value":
            return {"action": "raise", "amount": _raise_to(state, plan["_size"])}
        if plan["_kind"] == "bluff":
            _bluff_rng = random.Random(_spot_seed(state) ^ 0xB1FF5EED)
            if _bluff_rng.random() < plan["_freq"]:
                return {"action": "raise", "amount": _raise_to(state, plan["_size"])}

    base = _postflop_by_equity(state, eq, pos, can_check, board, rng)
    # ---- defensive: facing a bet only, flip ONLY marginal decisions (eq within
    # a small band of pot odds) on a confident read; byte-identical to baseline
    # everywhere else. Never touches strong hands or the checked-to (offense) path.
    if (not can_check) and v_stats and base.get("action") in ("call", "fold"):
        req = _required_equity(state)
        act = base["action"]
        if (DEF_FOLD_VS_VALUE and act == "call"
                and req <= eq < req + DEF_FOLD_DELTA
                and value_heavy_read(v_stats, raise_cap=DEF_VALUE_RAISE_CAP)):
            return {"action": "fold"}              # don't pay off a value-heavy / trapping villain
        if (DEF_CALL_VS_BLUFF and act == "fold"
                and req - DEF_CALL_DELTA <= eq < req
                and over_bluffer_read(v_stats, conf=DEF_CALL_CONF,
                                      min_n=DEF_MIN_ACTS, aggr_floor=DEF_AGGR_FLOOR)):
            return {"action": "call"}              # call down lighter vs a confirmed over-bluffer
    return base
