"""
================================================================================
V5c v2 — TUNED conservatism pass (single allowed adjustment after the v1 A/B).
v1 A/B on tight_heavy was mean A-B = -89.6 (t=-1.68), i.e. it slightly WORSENED
the field it targets — the bluffs were net-negative because the field is mixed
(trap_tag check-raises, mc_pot_odds calls with equity, folders already fold to
value bets). This v2 makes the module strictly more conservative, in one
coordinated direction (bluff less often, risk less per bluff):
    _AF_BLUFF_SIZE   0.50 -> 0.33   (smaller bet lowers break-even B/(P+B))
    _AF_FREQ_CAP     0.65 -> 0.40   (fire on fewer qualifying spots)
    _AF_SAFETY_MARGIN 0.12 -> 0.18  (require a higher observed fold rate to fire)
Pre-committed decision rule: re-A/B vs frozen V4.1 on the SAME seeds. KEEP only
if tight_heavy mean A-B is clearly positive AND station_heavy does not regress.
Otherwise shelve permanently (_ANTIFOLDER_ENABLED=False) — the bluff premise
does not pay in this field and V4.1 ships unchanged there. No second tune.
================================================================================

V5c — capped anti-overfolder exploit module (built on frozen v4.1 classifier).
Base: v4_1_classifier_retuned.py (UNMODIFIED). Adds ONLY a capped anti-folder
(small c-bet / steal) line. This is the one +aggression module, so it is the
most safety-gated: it can spew if miscalibrated, so every gate must hold before
it fires, and it is hard-capped in both frequency and size.

WHAT CHANGES (only when ALL gates below hold, and only with a weak/air hand we
would otherwise check): fire a SMALL c-bet / steal to fold out an overfolder.
Nothing else changes — value hands, calls, sizing, and the classifier are
byte-for-byte identical to v4.1.

GATES (every one must hold; otherwise behavior == v4.1):
  * _ANTIFOLDER_ENABLED is True;
  * the relevant opponent is a detected OVERFOLDER:
      - top_impl == "folding_bot" with confidence >= 0.70 AND lambda > 0.3, OR
      - observed faced_bet >= 25 and fold_vs_bet/faced_bet >= 0.70;
  * the opponent is NOT station / calling_bot / perma_all_in (never bluff these);
  * the table is effectively HEADS-UP (exactly one live opponent) — no big
    multiway bluffs;
  * the board is dry;
  * we are NOT facing a raise/bet (we only bluff into a checked pot; if the
    villain has shown aggression we do not bluff — "stop bluffing if raised");
  * BREAK-EVEN test passes: with bet size b into pot p, fold_needed = b/(p+b);
    only bluff if observed fold_vs_bet rate > fold_needed + safety margin;
  * a hard per-spot frequency cap (deterministic via spot RNG).

Explicitly NOT added: anti-perma logic, anti-station logic, anti-Monte-Carlo
logic, or any large strategy rewrite. Classification/collection logic is
unchanged from v4.1; this module only READS existing stats / LAST_READ.
Disable entirely with _ANTIFOLDER_ENABLED = False.
================================================================================
...
# ===========================================================================
# V5c — capped anti-overfolder exploit module (the ONLY behavior change vs v4.1)
# ===========================================================================
# The single +aggression module, so it is the most heavily gated. Reads existing
# stats / LAST_READ only; never mutates classification state. Hard caps on
# frequency and size; break-even gated; heads-up only; never vs sticky types.

_ANTIFOLDER_ENABLED = True         # flip to False to fully disable V5c

# Hard caps / knobs (conservative).
_AF_BLUFF_SIZE = 0.33               # SMALL c-bet/steal size (pot fraction)
_AF_FREQ_CAP = 0.40               # max share of qualifying spots we actually bluff
_AF_SAFETY_MARGIN = 0.18           # observed fold rate must beat break-even by this
_AF_MIN_FACED = 25                 # min faced_bet sample for the observed-rate trigger
_AF_FOLD_TRIGGER = 0.70            # observed fold-vs-bet rate that triggers (obs path)
_AF_CONF = 0.70                    # confidence needed for the classifier path
_AF_LAMBDA = 0.30                  # lambda needed for the classifier path

# Types we must NEVER bluff at, regardless of fold stats.
_AF_NEVER_BLUFF = ("station", "calling_bot", "perma_all_in")


def _current_opponent_id(state):
    """Relevant opponent id: the lone live opponent (heads-up), else the main
    villain from LAST_READ if still live. Read-only; never raises."""
    me = state.get("seat_to_act")
    live = []
    for p in state.get("players") or []:
        try:
...
def _opp_is_overfolder(bot_id):
    """True if this opponent qualifies as an overfolder AND is not a sticky type.
      classifier path : top_impl=='folding_bot', conf>=0.70, lambda>0.3 (THIS villain)
      observed path   : faced_bet>=25 and fold_vs_bet/faced_bet>=0.70
    Never True for station / calling_bot / perma_all_in. Pure read."""
    if not bot_id:
        return False
    # Hard exclusion of sticky / jam types via the current read for this villain.
    try:
    ...
    # Also exclude by raw stats: anyone calling a lot of bets is not an overfolder.
    if st:
        fb = st.get("faced_bet", 0)
        if fb >= 20 and (st.get("call_vs_bet", 0) / fb) >= 0.5:
            return False
        # observed-rate trigger
        if fb >= _AF_MIN_FACED and (st.get("fold_vs_bet", 0) / fb) >= _AF_FOLD_TRIGGER:
            return True
    # classifier trigger (requires high conf AND meaningful lambda for THIS villain)
    try:
        if LAST_READ.get("villain") == bot_id:
...
def _antifolder_should_bluff(state, board):
    """Master gate. Returns True iff a capped anti-folder bluff is justified at
    THIS spot. All conditions must hold; otherwise behavior stays v4.1."""
    if not _ANTIFOLDER_ENABLED:
        return False
    try:
        # Only ever bluff into a checked pot (we are not facing a bet/raise).
        # This also satisfies "stop bluffing if villain raises".
        if not state.get("can_check", False) or state.get("amount_owed", 0) > 0:
            return False
        # Heads-up only — no big multiway bluffs.
        ...
        if not _opp_is_overfolder(opp):
            return False
        # Break-even: with a SMALL bet, fold_needed = b/(p+b). Only bluff if the
        # opponent's OBSERVED fold rate clears break-even by the safety margin.
        pot = state.get("pot", 0)
        ...
        if _observed_fold_rate(opp) <= fold_needed + _AF_SAFETY_MARGIN:
            return False
        # Hard frequency cap (deterministic per spot).
...
def _safetag(state):
    ...
    af_bluff = _antifolder_should_bluff(state, board)   # V5c: False unless all gates hold
    ...
    eq = equity_vs_random(hole, board, n_opp, rng=rng)
    if eq is not None:
        return _postflop_by_equity(state, eq, pos, can_check, board, rng, af_bluff)
    # Fallback: V1 heuristic buckets.
    return _postflop_by_heuristic(state, hole, board, pos, can_check, rng, af_bluff)


def _postflop_by_equity(state, eq, pos, can_check, board, rng, af_bluff=False):
    req = _required_equity(state)
    ...
    if can_check:
        # V5c: capped anti-overfolder small c-bet/steal (all gates already
        # checked in af_bluff). Replaces the rare baseline steal in this spot.
        if af_bluff:
...
def _postflop_by_heuristic(state, hole, board, pos, can_check, rng, af_bluff=False):
    ...
    if can_check:
        # V5c: capped anti-overfolder small c-bet/steal (gates in af_bluff).
        if af_bluff:
            return {"action": "raise", "amount": _raise_to(state, _AF_BLUFF_SIZE)}
