...
BOT_NAME = "v24_meta_counter"
BOT_AVATAR = "robot_1"

# ============================================================
# V16 experimental flags — each isolates ONE change (the V14 pattern).
# ALL DEFAULT FALSE => decide() is byte-identical to the v16 baseline.
# A variant file / test harness enables one (or a combo) by setting the dict,
# e.g.  bot._V16_FLAGS["action_filter"] = True. None of these ships until it
# clears the floor rule on the real backtester; the constants below are
# placeholders pending June-2 calibration.
#   ev_call       (#1) monotonic pot-odds/EV call-or-fold rule with a
#                      realization discount, REPLACING the per-band call/fold
#                      thresholds when facing a bet. Touches CALL/FOLD only —
#                      never the bet/raise sizing (that was V12's regression).
#   spr           (#2) makes the existing commit/stack-off equity margin
#                      SPR-aware (lighter low-SPR, stickier high-SPR) instead
#                      of a flat +0.06.
#   action_filter (#3) street/action-specific villain range filters
#                      (call/raise/passive-raise; two-overcards flop-only)
#                      refining the coarse connect/strong modes.
#   blocker_bluff (#4) blocker-weights the EXISTING rare dry-board steal
#                      frequency (no new bluff range — that's rejected V12).
# ============================================================
_V16_FLAGS = {
    "ev_call": False,
    "spr": False,
    "action_filter": False,          # keep v16_noranges / README-validated behavior
    "blocker_bluff": False,
    "river_underbluff_fold": False,  # v20: avoid opponent-type-dependent river folds
    "ev_sanity_filter": True,       # v20: narrow universal fake-strength / huge-heat filter
    "nutted_value_sizing": True,    # v20: universal nut-region low-SPR value sizing
    "value_acceleration": True,    # v21: faster bb/100 value/protection sizing, no opponent reads
    "pressure_acceleration": True, # v22: cheap dead-money farming + premium squeezes, no opponent reads
    "turn_raise_respect": True,    # v23: attack first, respect large value-heavy turn raises
    "meta_counter_v24": True,     # v24: respect check-raises/large rivers, punish weak block/donk bets
}


def _flag(name):
    return bool(_V16_FLAGS.get(name))
...
# --- [archetype] Tier-1 extreme-aggression detection (cliff-gated, hard) ----
_ARCH_ALLIN_MIN, _ARCH_ALLIN_RATE = 5, 0.90
_ARCH_MANIAC_MINA, _ARCH_MANIAC_RATE = 10, 0.85

def _arch_is_maniac(stats):
    """True iff villain shoves ~always OR raises ~always => range ~random."""
    if not stats:
        return False
    a = stats.get("actions", 0); allins = stats.get("allins", 0)
    if allins >= _ARCH_ALLIN_MIN and a and allins / a >= _ARCH_ALLIN_RATE:
        return True
    if a >= _ARCH_MANIAC_MINA and (stats.get("raises", 0) + allins) / a >= _ARCH_MANIAC_RATE:
        return True
    return False

def _lone_villain_stats(state):
    vs = _live_villains(state)
    return PLAYER_STATS.get(vs[0][1]) if len(vs) == 1 else None
...
def _range_and_filter_for_villain(state, villain_seat, villain_bot_id):
    ...
    # [archetype Tier-1, HARD] perma all-inner / perma raiser -> range ~random,
    # not strong. Override to UNKNOWN (any-2 random) so we call favourites.
    if _arch_is_maniac(stats):
        return "UNKNOWN", "none"
    ...
    if board_present and info["raised_postflop"] and passive:
        ...
        return base, ("passive_raise" if _flag("action_filter") else "strong")
    ...
    if board_present and info["raised_postflop"]:
        return "THREEBET", ("raise_connect" if _flag("action_filter")
                            else "connect")
    ...
    cont_mode = "none"
    if board_present and info["continued_postflop"]:
        cont_mode = "call_connect" if _flag("action_filter") else "connect"
    if info["four_bet_pre"]:
...
def _combo_passes_action_filter(combo, board_c, mode):
    """Street/action-specific board filter (v16 #3). Only reached when the
    action_filter flag is on; refines the coarse connect/strong modes into
    per-action ones. Street is read from len(board_c) (3=flop, 4=turn,
    5=river). Modes:
        call_connect  : villain CALLED. flop keeps pair+/strong-draw/two
                        overcards-with-backdoor; turn keeps pair+/strong draw
                        (overcards gone); river keeps made pair+ only.
        raise_connect : villain (non-passive) RAISED. top pair+/combo or strong
                        draw/made — a value-leaning continue.
        passive_raise : confirmed-passive villain RAISED. two-pair+/set/overpair/
                        made flush-straight/monster draw only (no bare top pair).
    Fail-safe: on parse error keep the combo (the engine's 'never empty a real
    pool' guard prevents over-tightening)."""
    try:
        street = len(board_c)
        cr = [_CARD_STR.get(c, str(c)) for c in combo]
        br = [_CARD_STR.get(c, str(c)) for c in board_c]
        ...
        for r in all_ranks:
            counts[r] = counts.get(r, 0) + 1
        mult = sorted(counts.values(), reverse=True)
        trips_plus = bool(mult and mult[0] >= 3)
        two_pair = (mult.count(2) >= 2)

        suit_max = max((all_suits.count(s) for s in set(all_suits)), default=0)
        made_flush = suit_max >= 5
        flush_draw = suit_max == 4
        # Backdoor flush = three to a suit including at least one hole card.
        bd_flush = any((all_suits.count(s) == 3 and s in c_suits)
                       for s in set(c_suits))
        ...
        for r in all_ranks:
            ...
            if v:
                ...
                if v == 14:
                    vals.add(1)
        sv = sorted(vals)
        made_straight = oesd = gutshot = False
        for lo in range(1, 11):
            hits = sum(1 for v in sv if lo <= v <= lo + 4)
            if hits >= 5:
                made_straight = True
        ...
        pocket_pair = (c_ranks[0] == c_ranks[1])
        overpair = pocket_pair and _RANK_VAL.get(c_ranks[0], 0) > top_board
        paired_board = (c_ranks[0] in b_ranks) or (c_ranks[1] in b_ranks)
        pair_val = 0
        if paired_board:
            for hc in cr:
                if hc[0] in b_ranks:
                    pair_val = max(pair_val, _RANK_VAL.get(hc[0], 0))
        top_pair = paired_board and pair_val >= top_board
        made_pair_plus = (pocket_pair or paired_board or two_pair
                          or trips_plus or made_flush or made_straight)
        overcards = sum(1 for hc in c_ranks
                        if _RANK_VAL.get(hc, 0) > top_board)
        strong_draw = made_flush or flush_draw or oesd
        combo_draw = (flush_draw and oesd) or (flush_draw and overcards >= 1)
        monster_draw = combo_draw or (flush_draw and made_pair_plus)

        if mode == "passive_raise":
            return bool(two_pair or trips_plus or overpair or made_flush
                        or made_straight or monster_draw)

        if mode == "raise_connect":
            return bool(top_pair or two_pair or trips_plus or overpair
                        or made_flush or made_straight or combo_draw
                        or flush_draw or oesd)

        # call_connect, street-dependent.
        if street <= 3:                       # flop
            return bool(made_pair_plus or strong_draw
                        or (overcards >= 2 and (bd_flush or gutshot)))
        if street == 4:                       # turn
            return bool(made_pair_plus or strong_draw)
        return bool(made_pair_plus)           # river: made hand only
    except Exception:
        return True
...
def _equity_vs_multi_range(hole_c, board_c, opp_range_strs, rng,
    ...
    for i, rs in enumerate(opp_range_strs):
        ...
        if board_filter_modes is not None and i < len(board_filter_modes):
            ...
            mode = "none"   # [DECOMP noranges] disable board-aware narrowing (#1/#6)
        if rs == "UNKNOWN":
        ...
        if board_c and mode in ("connect", "strong",
            ...
            else:                              # v16 #3 action-specific modes
                filtered = [cb for cb in pool
                            if _combo_passes_action_filter(cb, board_c, mode)]
...
# ---- V22 universal pressure helpers ---------------------------------------

def _preflop_callers_after_first_raise(state):
    """Count preflop callers after the first raise and before any second raise.

    Used only for premium squeeze sizing. It is purely action-structure based,
    not opponent-type based.
    """
    try:
        seen_open = False
        callers = 0
        for rec in _reconstruct_hand(state.get("action_log") or []):
            if rec.get("street_idx") != 0:
                continue
            act = rec.get("action")
            if act in ("raise", "all_in"):
                if not seen_open:
...
def _v22_premium_squeeze_preflop(state, hand, history):
    """Bigger preflop squeeze with premiums only.

    This targets open + caller dead-money spots. We do not widen continues or
    jam-call ranges, so the added EV comes from premium extraction/isolation.
    """
    if not _flag("pressure_acceleration"):
        return None
    try:
        if state.get("street") != "preflop":
            return None
        if history.get("raises") != 1 or history.get("first_raiser_was_us"):
            return None
        if _is_facing_jam(state):
            return None
        if _preflop_callers_after_first_raise(state) < 1:
            return None
        if hand not in {"AA", "KK", "QQ", "AKs"}:
            return None
        target = _raise_to(state, 1.15)
        min_to = state.get("min_raise_to", 0) or 0
        cap = (state.get("your_stack", 0) or 0) + (state.get("your_bet_this_street", 0) or 0)
        if target < min_to:
            return None
...
def _preflop_v6(state):
    ...
    # v22: premium-only squeeze over open + callers. This is placed before
    # normal defense tables, but after jam/position context is known. It does
    # not change any non-premium or all-in-facing decisions.
    squeeze = _v22_premium_squeeze_preflop(state, hand, history)
    ...
    if history["raises"] >= 1 and committing and owed > 0:
        ...
        if eq >= req + _commit_margin(state, n_opp):
            return {"action": "call"}
        return {"action": "fold"}
...
def _postflop_pos_score(state):
    """Approximate postflop acting position in [0,1] from blind order and
    live seats, not absolute seat index. 0 = first to act, 1 = last to act.

    This is the only positional correction in v20. Preflop chart logic remains
    unchanged; postflop probes/value bets stop treating seat 5 as permanently
    late position. Uses only documented engine fields plus action_log blinds.
    """
    try:
        me = state.get("seat_to_act")
        players = state.get("players") or []
        ...
        for p in players:
            if not isinstance(p, dict):
                continue
            seat = p.get("seat")
            if seat is None or p.get("is_folded") or p.get("state") == "busted":
                continue
            active.append(seat)
        ...
        for _ in range(max(2, n)):
            if seat in active and seat not in order:
                order.append(seat)
            seat = (seat + 1) % n
        if me not in order or len(order) <= 1:
            return 0.5
...
def _safetag(state):
    ...
    pos = _position(state) if street == "preflop" else _postflop_pos_score(state)
    n_opp = _n_live_opponents(state)
    ...
    if street == "preflop":
        ...
        if facing and committing and owed > 0:
            ...
            if eq is None:
                eq = _preflop_equity_guess(cls, n_opp)
            req = _required_equity(state)
            if eq >= req + _commit_margin(state, n_opp):   # margin for domination
                return {"action": "call"}
            return {"action": "fold"}
    ...
    eq = _hero_equity(hole, board, state, n_opp, rng)
    if eq is not None:
        raw = _postflop_by_equity(state, eq, pos, can_check, board, rng)
        return _finals_hardened_filter(raw, state, eq, board)
    # Fallback: V1 heuristic buckets.
    raw = _postflop_by_heuristic(state, hole, board, pos, can_check, rng)
    return _finals_hardened_filter(raw, state, None, board)


# ---- v16 #2 (SPR-aware commitment) -----------------------------------------

def _spr_effective_chips(state):
    """Effective stack (chips) vs the largest live opponent, incl. chips
    already in this street — mirrors _effective_stack_bb's accounting."""
    me = state.get("seat_to_act")
    mine = (state.get("your_stack", 0) or 0) + \
        (state.get("your_bet_this_street", 0) or 0)
    opp = []
    for p in state.get("players") or []:
        try:
            if p.get("seat") == me:
            ...
            if p.get("is_folded") or p.get("state") == "busted":
                continue
            s = (p.get("stack", 0) or 0) + (p.get("bet_this_street", 0) or 0)
            if s > 0:
                opp.append(s)
        except (KeyError, TypeError):
            continue
    return min(mine, max(opp)) if opp else mine


def _commit_margin(state, n_opp):
    """Equity margin required ON TOP of pot-odds at a commitment/stack-off spot.
    Baseline is the v7 flat +0.06. With the spr flag on (#2), it scales with
    SPR: lighter at low SPR (you're pot-committed; calling off correct on less),
    stickier at high SPR (deep, more room to be dominated), plus a multiway
    bump. Default-off => returns exactly 0.06 (byte-identical)."""
    if not _flag("spr"):
        return 0.06
    pot = max(1, state.get("pot", 0) or 1)
    spr = _spr_effective_chips(state) / pot
    m = 0.06
    if spr <= 1.5:
        m -= 0.08
    elif spr <= 3.0:
    ...
    if (n_opp or 0) >= 2:
        m += 0.04
    return max(-0.04, min(0.14, m))


# ---- v16 #1 (pot-odds / EV call-or-fold with realization) ------------------

def _realization(state, pos, n_opp):
    """Conservative equity-realization multiplier for a call/fold leaf. <1 OOP
    and slightly lower multiway; 1.0 in position. Deliberately mild — this is a
    showdown-bound call, so the dominant lever is the risk premium, not this."""
    r = 1.0
    if pos is not None and pos <= 0.5:        # out of position
        r -= 0.06
    if (n_opp or 0) >= 2:
        r -= 0.04
    return max(0.80, r)
...
def _ev_call_or_fold(eq, state, pos, n_opp):
    """Monotonic pot-odds rule (#1): call iff realized equity beats the price
    plus a small risk premium. Only reached when the ev_call flag is on; it
    REPLACES the per-band call/fold thresholds (it never decides whether to
    raise). Killing the per-band seams also removes the 0.55-seam
    non-monotonicity (eq 0.54 calls, 0.56 folds)."""
    req = _required_equity(state)
    realized = eq * _realization(state, pos, n_opp)
    risk_premium = 0.03
    if realized >= req + risk_premium:
        return {"action": "call"}
    return {"action": "fold"}


# ---- v16 #4 (blocker-weighted dry-board steal frequency) -------------------

def _blocker_score(hole, board):
    """Small blocker heuristic (#4) for the EXISTING rare dry-board steal. High
    score = our card removal makes villain's strong/continuing combos less
    likely (and unblocks folds); negative = we block the busted draws villain
    would fold. Used only to re-weight a bluff frequency that already exists —
    NOT to add new bluffs (that's rejected V12 territory)."""
    try:
        ...
        score = 0.0
        for s in _SUITS:
            if b_suits.count(s) >= 3 and ("A" + s) in [c for c in hole]:
                score += 3.0                  # nut-flush blocker
        if "A" in ranks:
            score += 1.0
        if "K" in ranks:
            score += 0.5
        # On a two-flush board, holding that suit blocks villain's busted
        # flush draws (hands he'd fold) — a penalty for a bluff.
        for s in _SUITS:
            if b_suits.count(s) == 2 and s in suits:
                score -= 1.0
        return score
    except Exception:
        return 0.0
...
def _blocker_steal_prob(hole, board, base=0.18):
    """Map blocker score to a steal probability centered on the existing base
    frequency, so total bluff volume stays ~constant but shifts toward good
    blockers. Default-off callers never reach this."""
    s = _blocker_score(hole, board)
    p = base + 0.06 * s
...
def _postflop_by_equity(state, eq, pos, can_check, board, rng):
    req = _required_equity(state)
    ev = _flag("ev_call")
    n_opp = _n_live_opponents(state) if ev else 0
    hole = state.get("your_cards", []) or []
    # [river_underbluff_fold] fold more to BIG river bets/jams from a CONFIRMED
    # low-aggression villain (value-heavy aggression in an underbluffing field).
    # Gated on raise_rate<=0.18 so it never fires vs a maniac. Off => identical.
    if _flag("river_underbluff_fold") and not can_check and len(board) >= 5:
        _owed = state.get("amount_owed", 0); _pot = max(1, state.get("pot", 0) or 1)
        _lv = _lone_villain_stats(state)
        if _owed >= 0.66 * _pot and _lv:
            _a = _lv.get("actions", 0)
            _rr = (_lv.get("raises", 0) + _lv.get("allins", 0)) / _a if _a >= 12 else 1.0
            if _rr <= 0.18:
                req += 0.06 if _facing_raise(state) else 0.04

    # Facing-bet call/fold leaf: the EV rule (#1) when on, else the band's
    # original threshold. Defined per-band via the `default` decision passed in.
    def _leaf(default):
        return _ev_call_or_fold(eq, state, pos, n_opp) if ev else default
    ...
    if eq >= 0.72:
        ...
        # (#1 governs only the call/fold leaf, never the raise decision.)
        wet = not _board_is_dry(board)
        if eq >= 0.85:
            if rng.random() < 0.70:
                return {"action": "raise", "amount": _raise_to(state, 0.9)}
            return _leaf({"action": "call"})
        if eq >= 0.80:
            if rng.random() < 0.55:
    ...
    if eq >= 0.55:
        if can_check:
            ...
            return {"action": "check"}
        return _leaf({"action": "call"} if eq >= req + 0.05
                     else {"action": "fold"})
    ...
    if eq >= 0.42:
        if can_check:
            return {"action": "check"}
        return _leaf({"action": "call"} if eq >= req else {"action": "fold"})

    # Weak: mostly give up; rare dry-board steal in position.
    if can_check:
        if pos > 0.6 and _board_is_dry(board):
            # #4: blocker-weight this existing steal frequency when on; else
            # the flat 0.18.
            steal_p = (_blocker_steal_prob(hole, board, base=0.18)
                       if _flag("blocker_bluff") else 0.18)
            if rng.random() < steal_p:
                return {"action": "raise", "amount": _raise_to(state, 0.5)}
        return {"action": "check"}
    # Facing a bet with a weak hand: fold by default; the EV rule may continue
    # vs a small bet when realized equity beats the price (#1's overfold fix).
    return _leaf({"action": "fold"})
...
# ===========================================================================
# V20 — finals-hardened universal bb/100 patch
# ===========================================================================
# Built on the README-validated v16_noranges foundation. This deliberately does
# NOT re-enable the rejected board-aware range filters, ev_call, multiway
# caution, thin-value expansion, or opponent-type exploit branches.
#
# Universal additions only:
#   1) postflop position correctness fix above;
#   2) narrow fake-strength / huge-heat sanity filter;
#   3) restricted raise-war gate on river/huge-heat spots;
#   4) nut-region low-SPR value sizing.

_STREET_IDX = {"preflop": 0, "flop": 1, "turn": 2, "river": 3}


def _bb_amount(state):
    try:
        for a in state.get("action_log") or []:
            if isinstance(a, dict) and a.get("action") == "big_blind" and a.get("amount"):
                return max(1, int(a.get("amount")))
    except Exception:
        pass
    return 100
...
def _straight_window_hits(cards):
    ...
    best = 0
    for lo in range(1, 11):
        best = max(best, sum(1 for v in vals if lo <= v <= lo + 4))
    return best


def _board_profile(board):
    b = board or []
    rc = _rank_counts(b)
    sc = _suit_counts(b)
    max_rank = max(rc.values()) if rc else 0
    max_suit = max(sc.values()) if sc else 0
    paired = any(v >= 2 for v in rc.values())
    trips_on_board = any(v >= 3 for v in rc.values())
    three_flush = max_suit >= 3
    four_flush = max_suit >= 4
    five_flush = max_suit >= 5
    straight_hits = _straight_window_hits(b)
    four_straight = straight_hits >= 4
    made_straight_board = straight_hits >= 5
    river_completed_flush = False
    river_completed_straight = False
    if len(b) >= 5:
        prev = b[:-1]
        prev_sc = _suit_counts(prev)
        try:
            river_suit = b[-1][1]
        except Exception:
            river_suit = None
        if river_suit and prev_sc.get(river_suit, 0) == 2 and max_suit >= 3:
            river_completed_flush = True
        if _straight_window_hits(prev) < 4 and straight_hits >= 4:
            river_completed_straight = True
    danger = 0
    if paired:
        danger += 1
    if trips_on_board:
    ...
    if four_straight:
        danger += 2
    if made_straight_board:
        danger += 1
    if river_completed_flush:
        danger += 2
    if river_completed_straight:
        danger += 2
    if paired and three_flush:
        danger += 1
    return {
        "paired": paired,
        "trips_on_board": trips_on_board,
        "three_flush": three_flush,
        "four_flush": four_flush,
        "five_flush": five_flush,
        "four_straight": four_straight,
        "made_straight_board": made_straight_board,
        "river_completed_flush": river_completed_flush,
        "river_completed_straight": river_completed_straight,
        "danger": danger,
    }


def _hero_flush_rank(hole, board):
    cards = (hole or []) + (board or [])
    sc = _suit_counts(cards)
    flush_suits = [s for s, n in sc.items() if n >= 5]
    if not flush_suits:
        return None
    best = None
    for su in flush_suits:
        hero_vals = []
        for c in cards:
            try:
                if c[1] == su and c in (hole or []):
                    hero_vals.append(_RANK_VAL.get(c[0], 0))
            except Exception:
                pass
        if hero_vals:
            cand = max(hero_vals)
...
def _strategic_hand_bucket(hole, board):
    try:
        hole = hole or []
        board = board or []
        cards = hole + board
        if len(hole) != 2 or len(board) < 3:
            return "unknown"
        rc_all = _rank_counts(cards)
        rc_board = _rank_counts(board)
        counts = sorted(rc_all.values(), reverse=True)
        hole_vals = sorted((_RANK_VAL.get(c[0], 0) for c in hole), reverse=True)
        board_vals = sorted((_RANK_VAL.get(c[0], 0) for c in board), reverse=True)
        top_board = board_vals[0] if board_vals else 0
        board_paired = any(v >= 2 for v in rc_board.values())
        ...
        fr = _hero_flush_rank(hole, board)
        if fr is not None:
            return "nut_flush" if fr >= 13 else "low_flush"

        if _straight_window_hits(cards) >= 5:
            return "straight"

        trip_ranks = [r for r, n in rc_all.items() if n >= 3]
        if trip_ranks:
            for r in trip_ranks:
                hole_has = sum(1 for c in hole if c[0] == r)
                board_has = rc_board.get(r, 0)
                if hole_has == 2 and board_has >= 1:
                    return "set"
                if hole_has == 1 and board_has >= 2:
                    return "trips"
            return "trips"

        pair_ranks = [r for r, n in rc_all.items() if n >= 2]
        if len(pair_ranks) >= 2:
            pocket = hole[0][0] == hole[1][0]
            if pocket and board_paired:
                return "overpair_on_paired_board" if hole_vals[0] > top_board else "two_pair_with_board_pair"
            hole_match_ranks = {c[0] for c in hole if c[0] in rc_board}
            if board_paired:
                return "two_pair_with_board_pair"
            if len(hole_match_ranks) >= 2:
                return "real_two_pair"
            return "two_pair_with_board_pair"

        if len(pair_ranks) == 1:
            pr = pair_ranks[0]
            pv = _RANK_VAL.get(pr, 0)
            pocket = hole[0][0] == hole[1][0]
            if pocket:
                return "clean_overpair" if pv > top_board else "pocket_underpair"
            if rc_board.get(pr, 0) >= 2 and all(c[0] != pr for c in hole):
                return "board_pair_only"
            if pv >= top_board:
                return "top_pair"
            uniq = sorted(set(board_vals), reverse=True)
            if uniq and len(uniq) > 1 and pv >= uniq[1]:
                return "second_pair"
            return "weak_pair"
        return "air"
    except Exception:
...
def _postflop_raise_count(state):
    try:
        cur = _STREET_IDX.get(state.get("street", ""), 0)
        return sum(1 for rec in _reconstruct_hand(state.get("action_log") or [])
                   if rec.get("street_idx") == cur and rec.get("action") in ("raise", "all_in"))
    except Exception:
        return 0
...
def _v20_spr(state):
    try:
        pot = max(1, state.get("pot", 0) or 1)
        return _spr_effective_chips(state) / pot
    except Exception:
        return 99.0


def _v20_is_action_stackoff(action, state):
    try:
        ...
        if a == "raise":
            ...
            return cap > 0 and amt >= 0.90 * cap
    except Exception:
        pass
    return False


def _v20_downshift_medium_action(state, eq=None, hard=False):
    if state.get("can_check", False):
        return {"action": "check"}
    ...
    if not hard:
        if req <= 0.22:
            return {"action": "call"}
        if eq is not None and req <= 0.30 and eq >= req + 0.10 and not _is_facing_jam(state):
            return {"action": "call"}
    return {"action": "fold"}


def _v20_nutted_value_sizing(action, state, bucket, prof):
    """Universal, bb/100-oriented value extraction for nut region only."""
    if not _flag("nutted_value_sizing"):
        return action
    try:
        if state.get("street") == "preflop":
            return action
        if bucket not in {"full_house_plus", "nut_flush", "set"}:
            return action
        if bucket == "set" and (prof.get("danger", 0) >= 3 or prof.get("four_straight") or prof.get("four_flush")):
            return action
        spr = _v20_spr(state)
        can_check = state.get("can_check", False)
        owed = state.get("amount_owed", 0) or 0
        # At low SPR, capture the stack immediately with nut-region hands.
        if spr <= 2.2:
            return {"action": "all_in"}
        # Otherwise, size larger only when the original line already wanted to
        # continue or when checked to with true nut-region value.
        a = str(action.get("action", "")).lower().strip() if isinstance(action, dict) else ""
        if can_check and bucket in {"full_house_plus", "nut_flush"}:
            return {"action": "raise", "amount": _raise_to(state, 1.05)}
        if owed > 0 and a in {"call", "raise", "all_in"} and bucket in {"full_house_plus", "nut_flush"}:
            return {"action": "raise", "amount": _raise_to(state, 1.15)}
    except Exception:
        pass
    return action


def _v21_value_accelerator(action, state, bucket, prof):
    """Universal bb/100 value-acceleration layer.

    No opponent typing, no protect-lead mode, and no extra bluffing. It only
    increases sizing when the hero has structurally strong value or needs to
    deny equity with a vulnerable but strong made hand. The later v20 sanity
    filter still gets final veto on fake-strength / completed-river spots.
    """
    if not _flag("value_acceleration"):
        return action
    try:
        if state.get("street") == "preflop" or not isinstance(action, dict):
            return action
        a = str(action.get("action", "")).lower().strip()
        # Do not rescue folds. This is an accelerator for hands the base already
        # wants to continue with, or for checked-to value spots.
        if a == "fold":
            return action
        ...
        can_bet = can_check or owed <= 0
        pot = max(1, state.get("pot", 0) or 1)
        cap = (state.get("your_stack", 0) or 0) + (state.get("your_bet_this_street", 0) or 0)
        spr = _v20_spr(state)
        n_opp = _n_live_opponents(state)
        danger = prof.get("danger", 0)
        river_completed = bool(prof.get("river_completed_flush") or prof.get("river_completed_straight"))

        # v22: pot-control non-nut straights on paired/flushy textures. The
        # base action may still call or bet; the accelerator just stops treating
        # these straights like nut-region hands.
        if bucket == "straight" and (prof.get("paired") or prof.get("three_flush") or prof.get("four_flush")):
            return action

        nut_region = {"full_house_plus", "nut_flush"}
        strong_region = {"clean_overpair", "real_two_pair", "set", "straight", "nut_flush", "full_house_plus"}
        draw_danger = bool(prof.get("three_flush") or prof.get("four_straight"))

        # 1) Nut hands in already-large pots: convert to stack capture.
        if bucket in nut_region and cap > 0 and pot >= 0.45 * cap:
            return {"action": "all_in"}

        # 2) Low-SPR auto-pressure with strong made hands. This is still value
        # only; top pair and fake two-pair classes are deliberately excluded.
        if bucket in strong_region and spr <= 2.5 and danger <= 2 and not river_completed:
            # Clean overpairs and real two-pair are allowed at low SPR, but not
            # when the board is already four-flush/four-straight.
            if bucket in {"clean_overpair", "real_two_pair"} and (prof.get("four_flush") or prof.get("four_straight")):
                pass
            else:
                return {"action": "all_in"}

        # 3) Pot-size with nut advantage / near-nut value on flop/turn.
        if street in {"flop", "turn"} and bucket in {"set", "straight", "nut_flush", "full_house_plus"} and danger <= 2:
            if can_bet or a in {"call", "raise", "all_in"}:
                target = _raise_to(state, 0.95)
                if target > (state.get("current_bet", 0) or 0):
                    return {"action": "raise", "amount": target}

        # 4) Vulnerable overpair protection. Only when checked to/no bet faced:
        # charge draws; do not turn facing-heat overpairs into raises.
        if street in {"flop", "turn"} and bucket == "clean_overpair" and can_bet:
            if draw_danger and not (prof.get("four_flush") or prof.get("four_straight")):
                target = _raise_to(state, 0.85)
                if target > (state.get("current_bet", 0) or 0):
                    return {"action": "raise", "amount": target}

        # 5) Multiway no-free-card value pressure. This is aggression, not
        # caution: when we have real value and are checked to, price the field.
        if n_opp >= 2 and can_bet and bucket in strong_region:
            if bucket in {"clean_overpair", "real_two_pair"} and (prof.get("four_flush") or prof.get("four_straight") or river_completed):
                return action
            if bucket == "straight" and (prof.get("three_flush") or prof.get("paired")) and danger >= 3:
                return action
            target = _raise_to(state, 0.85)
            if target > (state.get("current_bet", 0) or 0):
                return {"action": "raise", "amount": target}
...
def _v22_hero_was_preflop_aggressor(state):
    """True if hero was the last preflop raiser."""
    try:
        return bool(_preflop_history(state).get("last_raiser_was_us"))
    except Exception:
        return False


def _v22_flop_checked_through(state):
    """True on turn/river if flop had actions and no bet/call/raise/all-in.

    We only use this for the turn delayed stab. If reconstruction is ambiguous,
    return False rather than inventing aggression.
    """
    try:
        saw_flop = False
        for rec in _reconstruct_hand(state.get("action_log") or []):
            if rec.get("street_idx") == 1:
                saw_flop = True
                if rec.get("action") in {"call", "raise", "all_in"}:
                    return False
        return saw_flop
    except Exception:
        return False
...
def _v22_pressure_accelerator(action, state, bucket, prof):
    """Cheap dead-money farming for unsophisticated/passive fields.

    This adds no river bluffing and no opponent typing. It only fires when no
    bet is faced, so future raises can still be folded by the existing logic.
    """
    if not _flag("pressure_acceleration"):
        return action
    try:
        ...
        if a == "fold":
            return action
        owed = state.get("amount_owed", 0) or 0
        can_bet = bool(state.get("can_check", False)) or owed <= 0
        if not can_bet:
            return action
        current_bet = state.get("current_bet", 0) or 0
        danger = prof.get("danger", 0) or 0
        river_completed = bool(prof.get("river_completed_flush") or prof.get("river_completed_straight"))

        # Do not add pressure with non-nut straights on paired/flushy boards.
        if bucket == "straight" and (prof.get("paired") or prof.get("three_flush") or prof.get("four_flush")):
            return action

        # 1) Flop c-bet pressure after hero was preflop aggressor. Dry boards,
        # cheap size, broad but not suicidal. If the base already bets larger,
        # leave it alone.
        if street == "flop" and _v22_hero_was_preflop_aggressor(state):
            if danger <= 1 and bucket in {"air", "weak_pair", "second_pair", "top_pair", "clean_overpair"}:
                target = _raise_to(state, 0.42)
                if target > current_bet and a in {"check", "call"}:
                    return {"action": "raise", "amount": target}

        # 2) Delayed turn stab after flop checked through. This farms dead
        # money versus passive fields. No scary completed river/river logic.
        if street == "turn" and _v22_flop_checked_through(state):
            if danger <= 2 and not river_completed and bucket in {
                "air", "weak_pair", "second_pair", "top_pair", "clean_overpair",
                "real_two_pair", "set", "straight", "nut_flush", "full_house_plus"
            }:
                target = _raise_to(state, 0.62)
                if target > current_bet and a in {"check", "call"}:
                    return {"action": "raise", "amount": target}
    except Exception:
        pass
...
def _v23_hero_aggressed_this_street(state):
    """True if hero has bet/raised/all-in on the current postflop street.

    Used to detect the exact meta spot: we attacked first on turn and a strong
    opponent applied a large raise. This is action-structure based only.
    """
    try:
        cur = _STREET_IDX.get(state.get("street", ""), 0)
        me = state.get("seat_to_act")
        if cur <= 0:
            return False
        return any(
            rec.get("street_idx") == cur
            and rec.get("seat") == me
            and rec.get("action") in {"raise", "all_in"}
            for rec in _reconstruct_hand(state.get("action_log") or [])
        )
    except Exception:
        return False
...
def _v23_last_aggressor_is_opponent(state):
    """True if the latest bet/raise/all-in this street came from another seat."""
    try:
        cur = _STREET_IDX.get(state.get("street", ""), 0)
        me = state.get("seat_to_act")
        last = None
        for rec in _reconstruct_hand(state.get("action_log") or []):
            if rec.get("street_idx") == cur and rec.get("action") in {"raise", "all_in"}:
                last = rec.get("seat")
        return last is not None and last != me
    except Exception:
        return False


def _v23_turn_raise_respect(action, state, bucket, prof, eq=None):
    """Counter-meta: attack first, but respect serious turn resistance.

    Qualifier II winners looked value-heavy in big pots. This does not reduce
    our own c-bets/stabs. It only vetoes continuing with fake/one-pair strength
    after we bet and face a large turn raise on a dangerous board.
    """
    if not _flag("turn_raise_respect"):
        return action
    try:
        if state.get("street") != "turn" or not isinstance(action, dict):
            return action
        a = str(action.get("action", "")).lower().strip()
        if a not in {"call", "raise", "all_in"}:
            return action
        owed = state.get("amount_owed", 0) or 0
        if owed <= 0 or state.get("can_check", False):
            return action
        if not _v23_hero_aggressed_this_street(state):
            return action
        if not _v23_last_aggressor_is_opponent(state):
            return action
        ...
        turn_bad_texture = (
            danger >= 2
            or prof.get("paired")
            or prof.get("three_flush")
            or prof.get("four_straight")
            or prof.get("four_flush")
        )
        if not turn_bad_texture:
            return action
        pot = max(1, state.get("pot", 0) or 1)
        bb = _bb_amount(state)
        facing_jam = _is_facing_jam(state)
        action_stackoff = _v20_is_action_stackoff(action, state)
        large_raise = facing_jam or action_stackoff or owed >= max(8 * bb, int(0.45 * pot))
        if not large_raise:
            return action

        vulnerable = {
            "weak_pair", "second_pair", "top_pair", "clean_overpair",
            "overpair_on_paired_board", "two_pair_with_board_pair"
        }
        # Real two-pair only loses acceleration on very dangerous boards; do not
        # overfold normal two-pair value versus one raise.
        if bucket == "real_two_pair" and danger >= 3:
            return _v20_downshift_medium_action(state, eq, hard=(facing_jam or action_stackoff))
        if bucket in vulnerable:
            return _v20_downshift_medium_action(state, eq, hard=(facing_jam or action_stackoff or danger >= 3))
        return action
    except Exception:
        return action
...
def _v24_bet_fraction(state):
    """Current facing bet as a fraction of the pot, using engine's pot field.

    Returns None when no bet is faced. This is intentionally structure-only:
    no opponent type, no historical reads.
    """
    try:
        owed = state.get("amount_owed", 0) or 0
        if owed <= 0 or state.get("can_check", False):
...
def _v24_meta_value_punish(action, state, bucket, prof):
    """Exploit weak/blocking bet sizes with value only.

    Counter-meta: weaker lines (tiny donk/block bets) are attacked with real
    value and nut-region hands. No bluffs, no type reads, no medium-hand heroics.
    """
    if not _flag("meta_counter_v24"):
        return action
    try:
        if not isinstance(action, dict) or state.get("street") == "preflop":
            return action
        a = str(action.get("action", "")).lower().strip()
        if a == "fold":
            return action
        if not _v24_bet_fraction(state):
            return action
        frac = _v24_bet_fraction(state)
        street = state.get("street")
        current_bet = state.get("current_bet", 0) or 0
        pot = max(1, state.get("pot", 0) or 1)
        bb = _bb_amount(state)

        # 1) River blocker-bet punish: raise nut-region versus small river bets.
        # Include only genuinely safe straights. Low/non-nut flushes are excluded.
        safe_nut_straight = (
            bucket == "straight"
            and prof.get("danger", 0) <= 1
            and not prof.get("paired")
            and not prof.get("three_flush")
            and not prof.get("four_flush")
        )
        if street == "river" and frac <= 0.35 and (bucket in {"full_house_plus", "nut_flush"} or safe_nut_straight):
            target = max(_raise_to(state, 1.10), current_bet * 3, state.get("min_raise_to", 0) or 0)
            return {"action": "raise", "amount": int(target)}

        # 2) Flop/turn tiny donk punish with strong value. This turns weak lead
        # sizes into larger value/protection raises, while leaving medium hands
        # alone and avoiding extreme textures.
        if street in {"flop", "turn"} and frac <= 0.30 and (current_bet <= max(4 * bb, int(0.35 * pot))):
            strong = {"clean_overpair", "real_two_pair", "set", "straight", "nut_flush", "full_house_plus"}
            if bucket not in strong:
                return action
            danger = prof.get("danger", 0) or 0
            if bucket == "clean_overpair" and (danger >= 3 or prof.get("four_flush") or prof.get("four_straight")):
                return action
            if bucket == "real_two_pair" and danger >= 3:
                return action
            if bucket == "straight" and (prof.get("paired") or prof.get("three_flush") or prof.get("four_flush")):
                return action
            if bucket == "set" and (prof.get("four_flush") or prof.get("four_straight")):
                return action
            target = max(_raise_to(state, 0.75), current_bet * 3, state.get("min_raise_to", 0) or 0)
            return {"action": "raise", "amount": int(target)}
    except Exception:
        return action
    return action
...
def _v24_flop_checkraise_respect(action, state, bucket, prof, eq=None):
    """Respect large wet-board flop check-raises after we c-bet.

    v22 adds more cheap flop c-bets. This prevents the obvious counter-leak:
    continuing too light when a stronger finalist check-raises a wet flop.
    """
    if not _flag("meta_counter_v24"):
        return action
    try:
        if state.get("street") != "flop" or not isinstance(action, dict):
            return action
        a = str(action.get("action", "")).lower().strip()
        if a not in {"call", "raise", "all_in"}:
            return action
        if state.get("can_check", False) or (state.get("amount_owed", 0) or 0) <= 0:
            return action
        if not _v23_hero_aggressed_this_street(state) or not _v23_last_aggressor_is_opponent(state):
            return action
        wet = (
            (prof.get("danger", 0) or 0) >= 2
            or prof.get("three_flush")
            or prof.get("four_straight")
            or prof.get("four_flush")
        )
        ...
        facing_jam = _is_facing_jam(state)
        action_stackoff = _v20_is_action_stackoff(action, state)
        large_raise = facing_jam or action_stackoff or owed >= max(6 * bb, int(0.45 * pot))
        if not large_raise:
            return action
        vulnerable = {"air", "weak_pair", "second_pair", "top_pair"}
        if bucket in vulnerable:
            return _v20_downshift_medium_action(state, eq, hard=(facing_jam or action_stackoff))
        # Clean overpairs are still value on many flops, but not enough to stack
        # off automatically on very wet textures versus a large check-raise.
        if bucket == "clean_overpair" and (prof.get("danger", 0) >= 3 or prof.get("three_flush") or prof.get("four_straight")):
            return _v20_downshift_medium_action(state, eq, hard=(facing_jam or action_stackoff))
    except Exception:
        return action
    return action
...
def _v24_river_large_bet_discipline(action, state, bucket, prof, eq=None):
    """Stricter discipline versus large river bets with medium/fake strength.

    Qualifier II big revealed losses were mostly one-pair/two-pair classes into
    value-heavy pressure. This handles the single-large-bet case, not only raise
    wars, while leaving nut-region uncapped.
    """
    if not _flag("meta_counter_v24"):
        return action
    try:
        if state.get("street") != "river" or not isinstance(action, dict):
            return action
        a = str(action.get("action", "")).lower().strip()
        if a not in {"call", "raise", "all_in"}:
            return action
        if state.get("can_check", False) or (state.get("amount_owed", 0) or 0) <= 0:
            return action
        if bucket in {"full_house_plus", "nut_flush"}:
            return action
        owed = state.get("amount_owed", 0) or 0
        pot = max(1, state.get("pot", 0) or 1)
        bb = _bb_amount(state)
        facing_jam = _is_facing_jam(state)
        large_bet = facing_jam or owed >= max(8 * bb, int(0.75 * pot))
        if not large_bet:
            return action
        dangerous = (
            (prof.get("danger", 0) or 0) >= 2
            ...
            or prof.get("four_flush")
            or prof.get("four_straight")
            or prof.get("river_completed_flush")
            or prof.get("river_completed_straight")
        )
        if not dangerous:
            return action
        medium = {
            "weak_pair", "second_pair", "top_pair", "clean_overpair",
            "overpair_on_paired_board", "two_pair_with_board_pair", "low_flush"
        }
        if bucket in medium:
            return _v20_downshift_medium_action(state, eq, hard=(facing_jam or prof.get("danger", 0) >= 3))
        # Real two-pair is still strong; only downshift it on very bad completed
        # river textures. This avoids reintroducing a broad defensive nerf.
        if bucket == "real_two_pair" and (prof.get("danger", 0) >= 4 or prof.get("river_completed_flush") or prof.get("river_completed_straight")):
            return _v20_downshift_medium_action(state, eq, hard=facing_jam)
    except Exception:
        return action
    return action

def _finals_hardened_filter(action, state, eq=None, board=None):
    """Universal finals hardening on top of v16_noranges.

    This is not a survival mode and does not use opponent typing. It only
    intercepts structurally bad huge-heat/fake-strength spots while preserving
    v16's fast bb/100 aggression elsewhere.
    """
    if not _flag("ev_sanity_filter"):
        return action
    try:
        if state.get("street") == "preflop" or not isinstance(action, dict):
            return action
        board = board if board is not None else (state.get("community_cards") or [])
        hole = state.get("your_cards") or []
        bucket = _strategic_hand_bucket(hole, board)
        prof = _board_profile(board)
        action = _v20_nutted_value_sizing(action, state, bucket, prof)
        action = _v21_value_accelerator(action, state, bucket, prof)
        action = _v22_pressure_accelerator(action, state, bucket, prof)
        action = _v24_meta_value_punish(action, state, bucket, prof)
        action = _v23_turn_raise_respect(action, state, bucket, prof, eq)
        action = _v24_flop_checkraise_respect(action, state, bucket, prof, eq)
        action = _v24_river_large_bet_discipline(action, state, bucket, prof, eq)
        a = str(action.get("action", "")).lower().strip()
        if a not in ("call", "raise", "all_in"):
            return action

        # Nut region stays uncapped by the defensive part of this wrapper.
        if bucket in {"full_house_plus", "nut_flush"}:
            return action
        ...
        pot = max(1, state.get("pot", 0) or 1)
        bb = _bb_amount(state)
        facing_bet = owed > 0 and not state.get("can_check", False)
        facing_jam = _is_facing_jam(state)
        raise_count = _postflop_raise_count(state)
        action_stackoff = _v20_is_action_stackoff(action, state)
        huge_heat = facing_jam or owed >= max(10 * bb, int(0.90 * pot)) or action_stackoff
        river_heat = street == "river" and facing_bet and (owed >= 0.70 * pot or facing_jam)
        raise_war = raise_count >= 2 and (street == "river" or huge_heat)
        river_completed = bool(prof.get("river_completed_flush") or prof.get("river_completed_straight"))

        # Medium made hands may call if correctly priced, but should not raise
        # river-completed flush/straight boards.
        medium_no_raise = {"top_pair", "clean_overpair", "overpair_on_paired_board",
                           "real_two_pair", "two_pair_with_board_pair", "set",
                           "trips", "straight", "low_flush"}
        if a in {"raise", "all_in"} and street == "river" and river_completed and bucket in medium_no_raise:
            return _v20_downshift_medium_action(state, eq, hard=False)

        # Narrow historical punt filters. No villain read exceptions.
        if bucket == "top_pair" and (river_heat or (huge_heat and prof.get("danger", 0) >= 3)):
            return _v20_downshift_medium_action(state, eq, hard=(facing_jam or prof.get("danger", 0) >= 4))

        if bucket == "overpair_on_paired_board" and prof.get("paired") and (river_heat or facing_jam or raise_war):
            return _v20_downshift_medium_action(state, eq, hard=(facing_jam or raise_war))

        if bucket == "two_pair_with_board_pair":
            got_there = (prof.get("three_flush") or prof.get("four_flush")
                         or prof.get("river_completed_flush") or prof.get("four_straight")
                         or prof.get("river_completed_straight"))
            # Includes the fake-strength paired-board class (e.g. QQ on K K x)
            # even when no flush/straight completed. Only huge river/jam/raise-war
            # heat is blocked, so normal bb/100 value aggression survives.
            paired_fake_heat = prof.get("paired") and (river_heat or facing_jam or raise_war)
            texture_fake_heat = got_there and prof.get("danger", 0) >= 3 and (river_heat or huge_heat or raise_war)
            if paired_fake_heat or texture_fake_heat:
                return _v20_downshift_medium_action(state, eq, hard=(facing_jam or prof.get("danger", 0) >= 4))

        if bucket == "low_flush" and prof.get("three_flush") and (huge_heat or river_heat or raise_war):
            return _v20_downshift_medium_action(state, eq, hard=facing_jam)

        # Restricted universal raise-war gate: only river/huge heat, and only
        # for non-nut/fake-strength classes. Normal flop/turn value wars remain.
        fake_or_medium = {"top_pair", "second_pair", "weak_pair", "clean_overpair",
                          "overpair_on_paired_board", "two_pair_with_board_pair",
                          "low_flush", "straight", "trips"}
        if raise_war and prof.get("danger", 0) >= 1 and bucket in fake_or_medium:
            return _v20_downshift_medium_action(state, eq, hard=True)
