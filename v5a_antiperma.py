"""
================================================================================
V5a — anti-perma-all-in exploit module (built on frozen v4.1 classifier).
Base: v4_1_classifier_retuned.py (UNMODIFIED). This file adds EXACTLY ONE
behavior change and nothing else.

THE ONE CHANGE: when we are facing an all-in or a very large COMMITTING bet AND
the aggressor is a detected perma-all-in opponent, we apply a TIGHTER equity
call filter (a larger required-equity margin) so we avoid marginal stack-offs
and only commit with strong holdings — letting the perma-jam bot punt into our
good hands. Outside that exact situation, behavior is byte-for-byte identical to
v4.1 (same preflop ranges, same postflop equity thresholds, same SafeTAG, same
sizing, same classifier).

Explicitly NOT added: anti-folder logic, anti-station logic, anti-Monte-Carlo
logic, new bluffing, or any general aggression change. The module is defensive,
capped, and trivially disablable via _ANTIPERMA_ENABLED = False.

Trigger (the aggressor we are facing must be a perma-bot):
  * high-confidence implementation read top_impl == "perma_all_in", OR
  * extreme observed jam rate: actions >= 15 and allins >= 4 and
    allins/actions > 0.25.

Collection/classification logic is unchanged from v4.1; this module only reads
the existing per-opponent stats and LAST_READ to decide whether to tighten the
call filter at a committing decision.
================================================================================
...
# ===========================================================================
# V5a — anti-perma-all-in exploit module (the ONLY behavior change vs v4.1)
# ===========================================================================
# Defensive, capped, easy to disable. Detects whether the AGGRESSOR we are
# facing is a perma-all-in opponent and, if so, returns an EXTRA required-equity
# margin to apply at committing decisions. It never changes any other line.

_ANTIPERMA_ENABLED = True          # flip to False to fully disable V5a
_ANTIPERMA_EXTRA_MARGIN = 0.12     # extra equity required over baseline at jams
                                   # (capped, so we still call clear monsters)


def _current_aggressor_id(state):
    """bot_id of the opponent whose bet/all-in we are currently facing, i.e. the
    last non-blind aggressive actor on this street who isn't us. None if unclear.
    Read-only over the public action log; never raises."""
    me = state.get("seat_to_act")
    seat_to_id = {}
    for p in state.get("players") or []:
        try:
    ...
    try:
        for a in state.get("action_log", []):
            act = a.get("action")
            seat = a.get("seat")
            if act in ("raise", "all_in") and seat != me:
                last = seat
    except Exception:
        return None
    return seat_to_id.get(last) if last is not None else None


def _is_perma_opponent(bot_id):
    """True if this specific opponent meets either perma-all-in trigger:
      (a) high-confidence implementation read top_impl == 'perma_all_in', or
      (b) extreme observed jam rate: actions>=15, allins>=4, allins/actions>0.25.
    Pure read of existing stats / LAST_READ; no side effects."""
    if not bot_id:
        return False
    ...
    # (b) extreme observed jam rate
    if actions >= 15 and allins >= 4 and (allins / actions) > 0.25:
        return True
    # (a) high-confidence perma_all_in read for THIS opponent
    try:
...
def _antiperma_extra_margin(state):
    """Return the extra required-equity margin to apply at THIS decision, or 0.0
    if V5a is inactive / not facing a perma opponent. Only ever returns >0 when
    we are facing that opponent's aggression; the caller still gates on the bet
    being committing, so this never affects small-bet or non-jam spots."""
    if not _ANTIPERMA_ENABLED:
        return 0.0
    try:
        aggressor = _current_aggressor_id(state)
        if _is_perma_opponent(aggressor):
            return _ANTIPERMA_EXTRA_MARGIN
    except Exception:
        return 0.0
    return 0.0
...
def _safetag(state):
    ...
    ap_margin = _antiperma_extra_margin(state)   # V5a: 0.0 unless facing perma
    ...
    if street == "preflop":
        ...
        if facing and committing and owed > 0:
            ...
            # V5a: tighten the filter vs a perma-jammer (ap_margin>0 only then).
            if eq >= req + 0.06 + ap_margin:   # margin for being dominated
                return {"action": "call"}
            return {"action": "fold"}
    ...
    # V5a: anti-perma committing-call filter. ONLY active when (a) V5a flagged a
    # perma aggressor (ap_margin>0), (b) we're facing a bet, and (c) that bet is
    # committing (large share of stack). In that exact spot, require a tighter
    # equity edge to call; otherwise fold and let them punt into our strong
    # hands. Outside this spot the code below is identical to v4.1.
    if ap_margin > 0 and not can_check:
        owed_pf = state.get("amount_owed", 0)
        stack_pf = state.get("your_stack", 0)
        committing_pf = owed_pf >= 0.6 * (stack_pf + state.get("your_bet_this_street", 0))
        if committing_pf and owed_pf > 0:
            e = eq if eq is not None else 0.5
            req = _required_equity(state)
            if e >= req + 0.05 + ap_margin:   # tighter than the usual +0.05
                return {"action": "call"}
            return {"action": "fold"}
