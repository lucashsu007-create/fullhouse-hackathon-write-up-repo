...
_RANGE_STRINGS = {
    ...
    "NUTTED":
        "JJ+, AKs, AKo",
    # ---- v13 additions (PLACEHOLDERS -- calibrate widths to D1 showdowns) ----
    # Open width tiers (selected by villain PFR):
    "OPEN_TIGHT":
        "55+, ATs+, KTs+, QTs+, JTs, AJo+, KQo",
    "OPEN_WIDE":
        "22+, A2s+, K7s+, Q8s+, J8s+, T8s+, 97s+, 86s+, 75s+, 65s, 54s, "
        "A7o+, K9o+, Q9o+, J9o+, T9o",
    # Continuation ranges (villain who has CALLED postflop bets -> air removed):
    "CONTINUE":
        "22+, A2s+, A9o+, K7s+, KTo+, Q8s+, QTo+, J8s+, JTo, T8s+, 97s+, "
        "86s+, 76s, 65s",
    "CONTINUE_TIGHT":
        "22+, A8s+, ATo+, K9s+, KJo+, QTs+, JTs, T9s, 98s",
    # Wide value+bluff 3-bet range for a confirmed LAG (mirror of the rock):
    "THREEBET_WIDE":
        "TT+, AJs+, AQo+, KJs+, QJs, JTs, T9s, 98s, 76s, 65s, A5s, A4s, A3s",
}

# ---- v13 range-construction flags (each prototype flips one) ----
USE_NARROW = False     # tighten a caller's range as they peel postflop bets
USE_WIDTH  = False     # scale an OPEN range by the villain's measured PFR
USE_LAG    = True      # widen a confirmed loose-aggressor's range (rock mirror)

WIDTH_LOOSE_PFR  = 0.28   # PFR above this -> OPEN_WIDE
WIDTH_TIGHT_PFR  = 0.12   # PFR below this -> OPEN_TIGHT
WIDTH_MIN_SAMPLE = 15     # min pfr_opp before trusting PFR
LAG_RAISE_RATE   = 0.22   # raises/actions above this -> aggressive
...
def _pfr_rate(stats):
    """Action-level PFR as a looseness proxy; None until enough sample."""
    if not stats:
        return None
    opp = stats.get("pfr_opp", 0)
    if opp < WIDTH_MIN_SAMPLE:
        return None
    return stats.get("pfr_yes", 0) / opp


def _villain_is_aggressive(stats):
    """Mirror of _villain_is_passive: confirmed loose-aggressive (high raise
    frequency). Conservative; returns False on insufficient sample."""
    if not stats:
        return False
    actions = stats.get("actions", 0)
    if actions < 20:
        return False
    raises = stats.get("raises", 0) + stats.get("allins", 0)
    return (raises / actions) > LAG_RAISE_RATE


def _villain_postflop_calls(state, villain_seat):
    """Count how many times this villain CALLED on a postflop street this hand
    (a proxy for 'peeled a bet' -> their range has shed air)."""
    try:
        recon = _reconstruct_hand(state.get("action_log") or [])
    except Exception:
        return 0
    return sum(1 for r in recon
               if r.get("seat") == villain_seat
               and r.get("street_idx", 0) >= 1 and r.get("action") == "call")


def _range_for_villain(state, villain_seat, villain_bot_id):
    """Range bucket for this villain. v7_m2 base ladder (limp->WIDE, open->OPEN,
    3bet->THREEBET, 4bet/passive-rocket->NUTTED), then three optional refinements
    (flag-gated): WIDTH scales an open by PFR, LAG widens a confirmed aggressor,
    NARROW tightens a passive caller who has peeled postflop bets."""
    ...
    # ---- base bucket (v7_m2 logic, unchanged) ----
    if showed_aggression and _villain_is_passive(stats):
        base = "NUTTED"
    elif info["four_bet_pre"]:
        base = "NUTTED"
    elif info["threebet_pre"]:
        base = "THREEBET"
    elif info["raised_pre"]:
        base = "OPEN"
    elif info["raised_postflop"]:
        base = "THREEBET" if not _villain_is_passive(stats) else "NUTTED"
    elif info["voluntary_pre"]:
        base = "WIDE"
    else:
        return "UNKNOWN"

    # ---- (#2) WIDTH: scale an OPEN by the villain's measured PFR ----
    if USE_WIDTH and base == "OPEN":
        pfr = _pfr_rate(stats)
        if pfr is not None:
            if pfr >= WIDTH_LOOSE_PFR:
                base = "OPEN_WIDE"
            elif pfr <= WIDTH_TIGHT_PFR:
                base = "OPEN_TIGHT"

    # ---- (#3) LAG: widen a confirmed aggressor's range (mirror of the rock) ----
    if USE_LAG and showed_aggression and _villain_is_aggressive(stats):
        if base == "THREEBET":
            base = "THREEBET_WIDE"
        elif base in ("OPEN", "OPEN_TIGHT"):
            base = "OPEN_WIDE"

    # ---- (#1) NARROW: tighten a passive caller who has peeled postflop bets ----
    if USE_NARROW and base == "WIDE":          # limp/call range only (CONTINUE strings are WIDE subsets)
        calls = _villain_postflop_calls(state, villain_seat)
        if calls >= 2:
            base = "CONTINUE_TIGHT"
        elif calls >= 1:
            base = "CONTINUE"
