"""
================================================================================
v6 — chart-based preflop on top of v4.1.

WHAT CHANGED vs v4.1 (preflop only; postflop and classifier are byte-identical):

  * Opens replaced. v4.1 used a 2-bucket positional rule (open `strong+` from
    pos <= 0.5, `playable+` from pos > 0.5, always pot-sized). v6 looks up the
    actual seat (UTG / HJ / CO / BTN / SB) and reads per-position open
    frequencies from the GTO Wizard 100bb 8-max chart, with per-seat open
    sizes (2.1bb / 2.2bb / 2.5bb / 3.5bb). Mixed-frequency cells (e.g. A6o
    opens 40% from CO) are sampled deterministically via the existing
    _spot_rng so paired A/B comparisons stay clean.

  * Position derived from action_log instead of seat_to_act / num_players.
    The engine doesn't broadcast the dealer; v6 walks back from the
    small_blind / big_blind actions to label every seat. Falls back to
    v4.1's _position() scalar if blinds aren't visible.

  * 3-bet defense expanded. v4.1 collapsed all openers into one decision:
    premium 3-bets, strong calls, rest folds. v6 uses opener-tier × defender-
    role tables (EARLY/MIDDLE/LATE × IP/OOP/BB), with bluff 3-bets from late
    position (A5s-A2s).

  * 4-bet defense added. When we open and someone 3-bets, v4.1 routes back
    through the same "facing raise" branch. v6 has a dedicated table: jam
    AA-KK, mix QQ and AKo, mostly fold AQs and below, with small A5s/A4s
    bluff 4-bets.

  * Short-stack mode added. Below 20bb effective stack, v6 switches to
    position-keyed jam / fold tables (20bb / 12bb / 8bb tiers). v4.1's
    "facing-all-in-commits-stack" equity filter is PRESERVED for the case
    where we face a jam — that logic was already good.

  * Heads-up SB override. When only 2 players remain in the hand, the 8-max
    SB chart (which limps 81%) is bypassed and v6 opens ~85% wide from the
    button.

WHAT'S UNCHANGED (byte-identical to v4.1):
  * equity engine (equity_vs_random, street budgets, deck cache)
  * postflop trees (_postflop_by_equity, _postflop_by_heuristic, dry-board
    bluff frequency, all the equity thresholds and sizings)
  * V3 PLAYER_STATS collection and reconstruction
  * V4 implementation classifier, IMPL_PROFILE weights, softmax,
    LAST_READ population (still collection-only)
  * sanitize / emergency / warmup hook / spot_rng / spot_seed

That isolation is deliberate: any EV delta from a paired A/B vs v4.1 is
attributable to the preflop replacement alone. The postflop tree was tuned
with 1M hands and isn't worth perturbing for V6.
...
def _preflop_v6(state):
    ...
    if history["raises"] >= 1 and committing and owed > 0:
        ...
        eq = equity_vs_random(hole, state.get("community_cards", []),
                              n_opp, rng=rng)
        if eq is None:
