...
_V16_FLAGS = {
    ...
    "sizing_jitter": True,   # deterministic +/-7% bet-size wobble (de-telling; no EV/decision change)   # fold more to big river bets/jams from low-aggr villains
}
...
def _raise_to(state, pot_fraction):
    ...
    stack = state.get("your_stack", 0)
    cap = stack + my_bet
    if _flag("sizing_jitter"):
        # De-telling only: deterministic +/-7% wobble on the pot fraction so the
        # bet SIZE stops leaking hand strength. Does not change which hands bet,
        # whether we bet, or any equity threshold -> EV-neutral. Off => exact line.
        pf = pot_fraction * (0.93 + 0.14 * _spot_rng(state).random())
        target = state.get("current_bet", 0) + int(pot * pf)
    else:
