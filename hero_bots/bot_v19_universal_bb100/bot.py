...
_V16_FLAGS = {
    ...
    "action_filter": True,
    "blocker_bluff": False,
    "river_underbluff_fold": False,  # universal-only v18: no opponent-type river adjustment
    "ev_sanity_filter": True,       # V18 universal bb/100: narrow EV-punt filter, no villain typing
}
...
def _range_and_filter_for_villain(state, villain_seat, villain_bot_id):
    """Universal action-based range and board filter for one live villain.

    v18 deliberately avoids opponent-type estimation here. No passive/maniac/
    station/folder reads are used. The only inputs are this hand's action line
    and objective position/open/3bet/4bet information:
      * postflop call/continue -> board-connected range filter,
      * postflop raise        -> raise-connected range filter,
      * preflop ladder        -> WIDE / OPEN_* / THREEBET / NUTTED.

    This preserves the universal correctness fix that a continuing range should
    not contain the same amount of pure air, without relying on noisy PLAYER_STATS
    type labels.
    """
    info = _villain_action_summary(state, villain_seat)
    board_present = len(state.get("community_cards", []) or []) >= 3

    # Preflop-only bucket = their range entering the flop. This is action-based,
    # not player-type-based.
    ...
    # Universal postflop raise filter. A raise is more selective than a call,
    # but we do not label villains as passive/maniac.
    if board_present and info["raised_postflop"]:
        base = pre_bucket if pre_bucket != "UNKNOWN" else "WIDE"
        return base, ("raise_connect" if _flag("action_filter") else "connect")

    # Universal postflop continuation filter.
    cont_mode = "none"
    if board_present and info["continued_postflop"]:
        cont_mode = "call_connect" if _flag("action_filter") else "connect"

    return pre_bucket, cont_mode
...
def _universal_preflop_jam_call(hand, state, n_opp):
    """Universal preflop all-in call override. It only forces calls with
    hands that are structurally strong versus an unknown jam range; it never
    widens based on opponent type and never forces marginal folds. Return
    True to call, None to let the equity engine/chart decide."""
    if not hand:
        return None
    req = _required_equity(state)
    multiway = (n_opp or 0) >= 2
    if multiway:
        always = {"AA", "KK", "QQ", "AKs", "AKo"}
        good_price = {"JJ", "AQs"}
    else:
        always = {"AA", "KK", "QQ", "JJ", "TT", "AKs", "AKo", "AQs"}
        good_price = {"99", "AJs", "AQo", "KQs"}
    if hand in always:
        return True
    if req <= 0.38 and hand in good_price:
        return True
    return None
...
def _preflop_v6(state):
    ...
    if history["raises"] >= 1 and committing and owed > 0:
        ...
        req = _required_equity(state)
        jam_override = _universal_preflop_jam_call(hand, state, n_opp)
        if jam_override is True:
            return {"action": "call"}
...
def _postflop_pos_score(state):
    """Approximate postflop acting position in [0,1] using blind seats and
    current live seats, not raw absolute seat index. 0 = first to act,
    1 = last to act. Uses only documented/public engine state fields.

    The old scalar seat_to_act/(n-1) made seat 5 permanently look 'late',
    which can create seat-dependent postflop aggression. Preflop chart logic
    still uses _seat_positions(); this is only for postflop probes/value bets.
    """
    try:
        me = state.get("seat_to_act")
        players = state.get("players") or []
        ...
        # Postflop order starts at the small blind seat and proceeds clockwise;
        # folded/busted seats are skipped. Heads-up is naturally SB then BB.
        order = []
        seat = sb
...
def _ev_call_or_fold(eq, state, pos, n_opp):
    """Universal monotonic pot-odds rule (#1): call iff realized equity beats
    the price plus a small structural premium. No opponent typing is used.
    The premium rises only on objectively difficult spots: river decisions,
    dangerous boards, and multiway pots. This removes threshold seams while
    preserving bb/100 pressure when the price is good."""
    req = _required_equity(state)
    realized = eq * _realization(state, pos, n_opp)
    board = state.get("community_cards", []) or []
    prof = _board_profile(board) if len(board) >= 3 else {"danger": 0}
    risk_premium = 0.03
    if state.get("street") == "river":
        risk_premium += 0.02
    if prof.get("danger", 0) >= 3:
        risk_premium += 0.03
    if (n_opp or 0) >= 2:
        risk_premium += 0.02
...
# ===========================================================================
# V17 — anti-bust stackoff governor
# ===========================================================================
# Conservative wrapper around the existing strategy. It does not create new
# aggression; it only downgrades calls/raises/all-ins in the exact high-risk
# patterns found in the Qualifier II hand histories: medium-strength hands on
# paired/flush/straight-danger boards facing heavy action or river heat.
...
def _board_profile(board):
    """Return board danger features. The score is intentionally coarse:
    0-1 mostly clean, 2 moderate, 3+ dangerous for non-nut stackoffs."""
    b = board or []
    rc = _rank_counts(b)
    sc = _suit_counts(b)
    ...
    if len(b) >= 5:
        ...
        river_suit = b[-1][1] if isinstance(b[-1], str) and len(b[-1]) >= 2 else None
...
def _strategic_hand_bucket(hole, board):
    """Strategic, not pure evaluator, bucket for anti-stackoff decisions."""
    try:
        hole = hole or []
        board = board or []
        cards = hole + board
        ...
        # Trips / sets before generic two-pair logic.
        ...
        if len(pair_ranks) == 1:
            ...
            # Paired board only, neither hole card paired.
            if rc_board.get(pr, 0) >= 2 and all(c[0] != pr for c in hole):
                return "board_pair_only"
            if pv >= top_board:
                return "top_pair"
            if board_vals and pv >= sorted(set(board_vals), reverse=True)[min(1, len(set(board_vals)) - 1)]:
                return "second_pair"
            return "weak_pair"
...
def _villain_spewy_enough(state):
    st = _lone_villain_stats(state)
    if not st:
        return False
    if _arch_is_maniac(st):
        return True
    a = st.get("actions", 0)
    if a < 12:
        return False
    aggr = (st.get("raises", 0) + st.get("allins", 0)) / a
    allin = st.get("allins", 0) / a
    return aggr >= 0.45 or allin >= 0.20


def _downshift_medium_action(state, eq=None, hard=False):
    """Replace a risky stackoff with the safest legal lower-variance action."""
    if state.get("can_check", False):
        return {"action": "check"}
    ...
    if owed <= 0:
        return {"action": "fold"}
    # Only keep calls that are cheap. If equity is available, require a real
    # cushion because these are precisely the dominated/non-nut buckets.
    if not hard:
        if req <= 0.22:
            return {"action": "call"}
...
def _universal_value_sizing(action, state, bucket, prof=None):
    """Universal bb/100 value extraction. No opponent typing.

    * full_house_plus / nut_flush: size up aggressively; jam at low SPR.
    * safe sets at low SPR: jam because worse value/draws still pay often and
      bb/100 rewards fast chip capture.
    * safe river medium-strong hands checked to us: add thin value instead of
      checking back. Top pair is excluded on purpose.
    """
    try:
        if not isinstance(action, dict) or state.get("street") == "preflop":
            return action
        prof = prof or _board_profile(state.get("community_cards") or [])
        ...
        # True nut region: maximize extraction.
        if bucket in {"full_house_plus", "nut_flush"}:
            if spr <= 3.0 or cap <= int(1.35 * pot):
                return {"action": "all_in"}
        ...
        # Safe low-SPR set: fast-scoop bb/100 line. Do not do this on boards
        # where the obvious flush/straight completion is present.
        if bucket == "set" and prof.get("danger", 0) <= 2 and spr <= 2.5:
            return {"action": "all_in"}

        # River thin value on safe boards when checked to. No top pair.
        if (state.get("street") == "river" and state.get("can_check", False)
                and a == "check" and prof.get("danger", 0) <= 2
                and bucket in {"clean_overpair", "real_two_pair", "set", "straight", "nut_flush"}):
            frac = 0.45 if bucket in {"clean_overpair", "straight"} else 0.60
            target = _raise_to(state, frac)
            if target >= 0.90 * cap:
                return {"action": "all_in"}
            return {"action": "raise", "amount": target}
...
def _universal_nut_value_sizing(action, state, bucket):
    # Back-compat wrapper for older call sites.
    return _universal_value_sizing(action, state, bucket)


def _river_completed_draw_no_raise(action, state, bucket, prof, eq=None):
    """Universal river completion blocker: when the river completes the most
    obvious flush/straight, medium made hands may call if priced in but should
    not raise/jam. No opponent typing."""
    try:
        if state.get("street") != "river" or not isinstance(action, dict):
            return action
        a = str(action.get("action", "")).lower().strip()
        if a not in ("raise", "all_in"):
            return action
        if not (prof.get("river_completed_flush") or prof.get("river_completed_straight")):
            return action
        if bucket in {"top_pair", "clean_overpair", "real_two_pair", "two_pair_with_board_pair",
                      "overpair_on_paired_board", "set", "trips", "straight"}:
            return _downshift_medium_action(state, eq, hard=False)
        return action
    except Exception:
        return action


def _anti_bust_filter(action, state, eq=None, board=None):
    """bb/100-oriented EV sanity filter.

    This is deliberately NOT a survival / protect-lead governor.  The match
    data showed Pantheon wins a lot of its EV by taking big medium-strength
    pots quickly, which is exactly what bb/100 rewards.  Therefore this wrapper
    only blocks the historically bad, high-danger stackoff buckets:
      * top pair facing huge river / high-danger heat,
      * fake two-pair created by a paired board on very dangerous runouts,
      * low / non-nut flush facing heavy action.

    It does not globally nerf clean overpairs, sets, real two-pair, straights,
    or aggression while ahead in chips. It is universal-only: no station/folder/
    maniac/spew typing is used for decisions.
    """
    ...
    try:
        ...
        a = str(action.get("action", "")).lower().strip()
        if a not in ("check", "call", "raise", "all_in"):
            return action
        ...
        # Heavy action definition is intentionally strict.  bb/100 wants us to
        # keep taking profitable big pots, so do not downshift routine value.
        cap = (state.get("your_stack", 0) or 0) + (state.get("your_bet_this_street", 0) or 0)
        raw_amt = action.get("amount", 0) if a == "raise" else 0
        try:
            raw_amt = int(raw_amt or 0)
        except Exception:
            raw_amt = 0
        action_stackoff = (a == "all_in") or (a == "raise" and cap > 0 and raw_amt >= 0.90 * cap)
        huge_bet = facing_jam or owed >= max(10 * bb, int(0.90 * pot)) or action_stackoff
        river_heat = street == "river" and facing_bet and (owed >= 0.70 * pot or facing_jam)
        raise_war = raise_count >= 2

        # Nutted / safe strong value extraction comes before defensive gates.
        if bucket in {"full_house_plus", "nut_flush"} or (bucket == "set" and prof["danger"] <= 2):
            up = _universal_value_sizing(action, state, bucket, prof)
            if up is not action:
                return _sanitize(up, state)

        # River completion blocker: medium made hands may call, but do not
        # raise/jam when the river completes the obvious flush/straight.
        no_raise = _river_completed_draw_no_raise(action, state, bucket, prof, eq)
        if no_raise is not action:
            return no_raise

        # Universal multiway caution: multiway + dangerous board + heavy action
        # objectively lowers medium-hand realization. This only blocks stackoff
        # structures, not normal bets/calls.
        if n_opp >= 2 and prof["danger"] >= 2 and (huge_bet or raise_war or action_stackoff):
            multiway_bad = {"top_pair", "second_pair", "weak_pair", "board_pair_only",
                            "overpair_on_paired_board", "two_pair_with_board_pair",
                            "low_flush", "pocket_underpair", "air"}
            if bucket in multiway_bad:
                return _downshift_medium_action(state, eq, hard=(facing_jam or action_stackoff))

        # Universal raise-war gate: if this street has already been reopened
        # multiple times, do not stack off non-nut / fake-strength holdings.
        # This is not a survival rule; it only blocks the highest-risk action
        # structure independent of villain identity.
        if raise_war and (huge_bet or action_stackoff or a in ("raise", "all_in")):
            medium_raise_war_buckets = {
                "top_pair", "second_pair", "weak_pair", "board_pair_only",
                "overpair_on_paired_board", "two_pair_with_board_pair",
                "low_flush", "pocket_underpair", "air"
            }
            if bucket in medium_raise_war_buckets:
                return _downshift_medium_action(state, eq, hard=(facing_jam or action_stackoff))
            if bucket == "straight" and (prof["three_flush"] or prof["paired"] or prof["four_flush"]):
                return _downshift_medium_action(state, eq, hard=(facing_jam or action_stackoff))
            if bucket == "real_two_pair" and prof["danger"] >= 4:
                return _downshift_medium_action(state, eq, hard=(facing_jam or action_stackoff))
            if bucket in {"set", "trips"} and (prof["four_straight"] or prof["four_flush"] or prof["river_completed_straight"] or prof["river_completed_flush"]):
                return _downshift_medium_action(state, eq, hard=(facing_jam or action_stackoff))

        # 1) Top pair was the clearest negative big-pot bucket.  Do not kill
        # ordinary top-pair value.  Only block huge river / very dangerous heat,
        # No villain-type exceptions in v18 universal-only.
        if bucket == "top_pair":
            if river_heat or (huge_bet and prof["danger"] >= 3):
                return _downshift_medium_action(state, eq, hard=(facing_jam or prof["danger"] >= 4))

        # 2) Fake two-pair from a paired board, especially when flush/straight
        # danger exists.  This targets QQ on K K x, AK on paired/flushy boards,
        # etc.  Real two-pair is intentionally NOT included.
        if bucket in {"two_pair_with_board_pair", "overpair_on_paired_board"}:
            board_got_there = (prof["three_flush"] or prof["four_flush"]
                               or prof["river_completed_flush"]
                               or prof["four_straight"]
                               or prof["river_completed_straight"])
            # Paired-board fake strength was a real leak even without an added
            # flush/straight completion: e.g. QQ on K K x or KK on Q Q x.  For
            # bb/100 we still allow normal aggression, but not huge river heat /
            # raise wars with a hand that is strategically just a bluffcatcher.
            paired_fake_heat = (bucket == "overpair_on_paired_board"
                                and prof["paired"] and (river_heat or facing_jam or raise_war))
            texture_fake_heat = (board_got_there and prof["danger"] >= 3
                                 and (river_heat or huge_bet or raise_war))
            if paired_fake_heat or texture_fake_heat:
                return _downshift_medium_action(state, eq, hard=(facing_jam or prof["danger"] >= 4))

        # 3) Low / non-nut flushes are still value hands.  Only downshift versus
        # huge heat.  Three-flush boards count: the historical 9-high-flush loss
        # was on a normal three-flush board, not only a four-flush/paired board.
        if bucket == "low_flush":
            severe_flush_spot = prof["three_flush"] or prof["paired"] or prof["four_flush"] or prof["five_flush"]
            if severe_flush_spot and (huge_bet or river_heat or raise_war):
                return _downshift_medium_action(state, eq, hard=facing_jam)

        return _sanitize(_universal_value_sizing(action, state, bucket, prof), state)
    except Exception:
        return action
