...
>
> Paired vs V7-M2 (fix), CI lower bound ≥ 0 ⇒ PASS:
>
> | field | `noranges` − V7-M2 | `nr_evsize` − V7-M2 |
> |---|---|---|
> | broad_mix | **+0.88** [+0.40,+1.36] all+ | **+1.72** [+1.35,+2.08] all+ |
> | broad_mix2 | +1.37 [+1.03,+1.70] all+ | +0.89 [+0.43,+1.35] |
> | exploiter_mix | +1.16 [+0.35,+1.98] | +1.04 [+0.38,+1.70] |
> | polar3bet | **+2.73** [+2.26,+3.20] all+ | +0.87 [+0.31,+1.43] |
> | pressure_field | +1.20 [+0.58,+1.82] all+ | +1.39 [+0.87,+1.92] all+ |
>
> **The broad_mix "−6.0 / −2.5 bleed" was a LOW-POWER ARTIFACT.** At 5 seeds × 40–50 matches it read as a floor-failing regression; at 10 seeds × 100 × 2500 it is **positive** for both upgrades (noranges +0.88, nr_evsize +1.72, all 10 seeds +). The decomposition's *mechanism* (board-ranges hurt vs stations) was correct, but the **aggregate ship verdict built on the noisy broad_mix numbers was wrong** — the bleed was too small to survive proper power, and the other fixes more than cover it. This is the project's own thesis enforced: *a floor conclusion drawn below the power bar is not a floor conclusion.* The corrections in the notices below about per-component **mechanism** stand (denominator inert; cold-4bet a gain; pos-opens a drag); the **"fails the floor / ship V7-M2"** conclusions in them are **withdrawn.**
>
> **RECOMMENDED SHIP: `v16_noranges`.** Clears the floor in all 5 fields, every CI lower bound > 0; largest single edge of any candidate (polar3bet +2.73); and it is the *lower-risk* diff (one component removed from a tested baseline). `v16_nr_evsize` also clears all 5 (its edge is biggest on stations, +1.72 broad_mix) and is the **anti-station hot-swap**; it is weaker on polar3bet (+0.87 vs +2.73) and carries the extra `ev_call` surface area. **V7-M2 (fix) remains the ultra-safe fallback.**
>
> **Honest scope:** edges are real but small (~1–2.7 bb/100; CIs exclude 0 at 2.5M hands), and all five fields are panel bots we wrote — robust *across the test populations*, not a guarantee vs an unknown qualifier archetype. Final upload still goes through `harden_scan.py`; the hot-swap (V7-M2 / nr_evsize) stays the safety net. Timing/safety gate also confirmed by these batches: 0 errors, `fold_on_timeout` never triggered at full 2500-hand length.

> **Update — 30 May 2026 (V16 wave).** A third wave corrected a latent **equity-engine bug** (the Monte-Carlo denominator counted dead/conflicted trials, deflating equity) plus seven other foundation fixes, and tested four flag-gated experimental changes. In paired AB the **V16 foundation beats V7-M2 (fix) by +3.8 to +4.2 bb/100 on the mean** — the largest, most consistent gain in the project from a pure input/correctness fix — **but it does not yet clear the floor rule**: it has negative floors in 3–4 high-variance fields that are unresolved at 3 seeds. Of the four flags, `action_filter` bled outright (the V7.2 failure re-confirmed), `spr` and `blocker_bluff` were inert, and `ev_call` is +mean/−floor. ~~**The D1 ship decision is unchanged: V7-M2 (fix).**~~ **[WITHDRAWN — see POWERED RESULT at top: `v16_noranges` is the validated ship.]** See [The equity-denominator fix](#the-equity-denominator-fix) and [V16 — foundation and flag evaluation](#v16--foundation-and-flag-evaluation).
>
> > **CORRECTION — 30 May 2026 (decomposition).** A component-by-component decomposition of the V16 foundation on the **real backtester** (direct paired AB, 5 seeds; see `v16_decomposition_results.md`) **supersedes several claims in the V16-wave update above and the sections it links.** Reduced power (5 seeds, 40–50×200–250 — below the 100×500×10 bar), but the large effects are unanimous-sign and the inert one is exact. Summary:
> > 1. **The equity-denominator fix (#2) is INERT, not "load-bearing."** Full-vs-denominator-reverted = **exactly 0 chips over 12,500 paired hands** (byte-identical play). V7-M2 *already* has the same inner 20-retry combo-salvage loop, so dead trials ≈ 0 in both and the accounting fix corrects a ~0 bias. Every claim below that calls #2 "the load-bearing component" / "the second-largest correctness fix" / "the cleanest single thing to promote" is **withdrawn.**
> > 2. **cold-4bet (#3) is a GAIN, not the negative-floor suspect.** Direct paired effect **+9.83 [+5.0,+14.7] (all 5 seeds +) on exploiter_mix, +3.92 [+0.5,+7.4] on broad_mix, ~0 on polar3bet.** The "mechanistic hypothesis"/"leading suspect" framing below is **wrong** — keep cold-4bet.
> > 3. **The negative floor is the board-aware range filter (#1/#6).** Isolated effect **−7.67 [−11.2,−4.1] (all 5 seeds −) on broad_mix** (the V7.2 lesson in chips: the "connect" filter strips *air* that calling stations genuinely continue with). Floors are **resolved**, not "unresolved at 3 seeds": full vs V7-M2 is exploiter_mix **+6.2** but broad_mix **−5.95 [−9.3,−2.6]** (real regression, traced to #1/#6).
> > 4. **pos-opens (#5) is a small drag/wash, not a help:** **−1.57 [−2.7,−0.5] (all 5 seeds −) on exploiter_mix**, +0.20 on broad_mix.
> > 5. **At 5-seed power, no single static config cleared the floor across all three fields** — looked like a population-specific (Pareto) result. **[UPDATED by the POWERED RESULT at top: at 100×2500×10 this is RESOLVED — both `v16_noranges` and `v16_nr_evsize` clear the floor in all 5 fields, including broad_mix. The "grazes/fails on broad_mix" reading here was a low-power artifact.]** Best candidate **`v16_noranges`**; `v16_nr_evsize` (adds size-aware `ev_call`) also passes and is the anti-station hot-swap. The arms (`v16_noranges`, `v16_nr_evsize`, `v16_coldonly`, `v16_nocold`, `v16_nothin`, `v16_noopens`, `v16_denomrev`) are in `hero_bots/`.

V7 → V7-M2 promoted (postflop marginal-cutoff tightening). V7.2 (street-narrowing) and V10 (squeezer-defense) families evaluated and rejected. The single largest gain in the project came from fixing one inverted boolean in `_preflop_history` that had rendered `_four_bet_branch` dead code — see [The dead-gate fix](#the-dead-gate-fix). ~~The second-largest came from fixing the equity-MC denominator (V16).~~ **[Corrected — see notice above: the denominator fix is inert (0 chips). The V16 foundation's actual gain comes from cold-4bet (#3); its actual bleed is the board-range filter (#1/#6).]**

A second wave of work (V11–V14) extended the search into opponent-conditional exploits, range-construction refinements, balance/sizing fixes, and a complete postflop **leak audit**, alongside two **absolute validation runs** against a static reference field and a hyper-aggressive pressure field, and the construction of the **patch-window toolchain**. Every V11–V14 family was rejected or deferred for the same structural reason the earlier overlays were. The validation runs confirm V7-M2 (fix) is robust across the full style spectrum. The toolchain is built and waiting for the June 2 hand-history drop. A third wave (V16) corrected the equity-MC denominator bug, bundled it with seven other foundation fixes, and tested four flag-gated changes. ~~**None of this changes the D1 ship decision: V7-M2 (fix) as-is.**~~ **[UPDATED — see POWERED RESULT at top: the V16-derived `v16_noranges` (foundation minus the board-range filter) was validated at 100×2500×10 to beat V7-M2 (fix) in all 5 fields and is now the recommended ship; V7-M2 (fix) is the fallback.]**
...
| **V16 (foundation)** | **Equity-MC denominator fix (#2) + board-aware ranges (#1/#6) + position-split opens (#5) + cold-4bet defense (#3) + thin value raises (#4) + jam/sanitize safety fixes (#7/#8)** | Postflop equity + preflop | **Candidate — +mean but FAILS floor: bleeds broad_mix −6.0; bleed isolated to board-ranges #1/#6, #2 inert, cold-4bet #3 a gain (see correction notice). Best subset `v16_noranges`/`v16_nr_evsize` → rig.** |
...
**Why it matters and why it's the "right kind" of fix.** This is an *input* error to a uniform policy — the robust class to correct (the project's recurring lesson: improving the input is robust; overriding the policy on a read bleeds). Unlike the dead gate (a preflop-logic bug), this one silently mis-valued every postflop hand. The entire V7-M2 postflop validation predates it, so V7-M2's absolute numbers were produced on a downward-biased equity engine. ~~The fix is the load-bearing component of the V16 foundation~~ **[CORRECTED — see notice at top: in paired play this fix is INERT (exactly 0 chips / byte-identical play), because V7-M2 already runs the same 20-retry combo-salvage loop that drives dead trials to ≈0. The bias it corrects is real in principle but ~0 in practice. The V16 foundation's gain comes from cold-4bet (#3), not this. The text below is retained for the record but the "load-bearing" framing is withdrawn.]**; the other seven V16 fixes are smaller or field-specific (below).
...
2. **Calling wider against 3-bets is a leak, not a fix.** Reaching more OOP postflop spots with weak ranges loses more than rare call-and-win recovers. The widest variant (sqz_w) lost the most. *(V16 note: the V16 foundation's cold-4bet branch (#3) was suspected of the mirror risk — but the decomposition disproves it: #3 is a net gain (+9.8/+3.9/~0); the V16 bleed is the board-range filter #1/#6. See correction notice at top.)*
...
1. **`#2` equity-MC denominator** — ~~the load-bearing fix~~ **[CORRECTED: inert in play — 0 chips; see top notice]**; see [its section](#the-equity-denominator-fix).
...
**Mechanistic hypothesis for the negative floors — DISPROVEN; corrected below.** *(Original hypothesis, retained for the record: the cold-4bet branch (#3) makes the bot call/4-bet some 3-bets V7-M2 always folded, reaching OOP postflop spots the equity logic was never tuned for, and this drives the negative floors.)* **The decomposition refutes this: cold-4bet (#3) is a GAIN (+9.8 exploiter / +3.9 broad / ~0 polar, all-positive on the first two). The negative floor on broad_mix is the board-aware range filter (#1/#6), isolated at −7.67 (all 5 seeds −): the "connect" filter strips air that calling stations genuinely continue with — the V7.2 lesson, in chips. (pos-opens #5 is a small drag, −1.57 on exploiter; thin-value #4 is a wash.)** The clean test was run (see `v16_decomposition_results.md`); the remaining open question is the borderline `broad_mix` magnitude at full power.
...
**Equity-MC denominator bias — RESOLVED (V16), but INERT in play (decomposition).** `_equity_vs_multi_range` counted dead/conflicted trials in the denominator, deflating equity worst in tight/multiway spots; fixed by separating attempts from successful trials. ~~The second-largest correctness fix in the project; the cleanest single thing to promote.~~ **[CORRECTED — see top notice: full-vs-denominator-reverted is exactly 0 chips over 12,500 paired hands. V7-M2 already had the retry-salvage loop, so this changes nothing in play. Not a promote lever.]**
...
**Hidden postflop leaks from newly-reached states — MEASURED (V16/decomposition).** Because the bot now actually calls some 3-bets it previously always folded (V7-M2 4-bet defense; and V16's cold-4bet branch #3), it reaches OOP postflop spots the equity logic was never tuned against. **[CORRECTED — cold-4bet #3 was the suspect here but the decomposition shows it is a net gain (+9.8/+3.9/~0); the broad_mix negative floor is the board-range filter #1/#6 (−7.7, all 5 seeds −), not the newly-reached cold-4bet states.]** Still worth a powered confirmation at ≥10 seeds (`run_powered.sh`).
...
**Ship decision (deadline Mon 1 Jun 12:00 UK): `v16_noranges` — VALIDATED.** Two full-power batches (100×2500×10, 2.5M hands/bot/field, 0 errors) show `v16_noranges` (the V16 foundation with the bleeding board-range filter #1/#6 removed) beats V7-M2 (fix) in **all five fields** — broad_mix +0.88, broad_mix2 +1.37, exploiter_mix +1.16, polar3bet +2.73, pressure_field +1.20, every CI lower bound > 0. `v16_nr_evsize` also clears all five (anti-station hot-swap, +1.72 broad_mix). The earlier "V16 foundation FAILS the floor (broad_mix −6.0)" reading was a **5-seed low-power artifact**, overturned at full power. **Ship `v16_noranges`; keep V7-M2 (fix) as the ultra-safe fallback and `v16_nr_evsize` as the station hot-swap.** Note the *full* foundation (`v16.5`, with #1/#6 still in) was the thing that failed broad_mix; the promote is specifically the foundation **minus** that component. The `harden_scan.py` gate (real ban list + 3.10 check) passes clean on v7_m2, v16_noranges, v16_nr_evsize; timing/safety confirmed by the batches (0 errors, `fold_on_timeout` never fired at 2500-hand length).
...
2. **Decompose the foundation** to separate the safe fix from the suspects — **DONE on the real backtester (5 seeds; `v16_decomposition_results.md`).** Findings: `#2` denominator inert (0 chips); `#3` cold-4bet a gain (+9.8/+3.9/~0); `#1`/`#6` board-ranges the bleed (−7.7 on broad_mix); `#5` pos-opens a small drag; `#4` thin-value a wash. **Remaining work is powered confirmation, not discovery:** run `run_powered.sh` (100×500×10) to resolve `v16_nr_evsize` (= `noranges` + size-aware `ev_call`) on exploiter_mix — broad/polar are already clearly positive. Promote iff all fields clear the floor at full power.
...
**Submission rule:** ship the version with the highest worst-field floor, not the highest mean. V7-M2 (fix) clears that bar; among the extension families, **`v16_noranges` and `v16_nr_evsize` now also clear it — validated at full power (100×2500×10) to beat V7-M2 in all five fields, lowest worst-field margin +0.88 bb/100, every CI > 0.** The earlier families still fail: V8/V9/V11 bleed on the floor via false positives, V12 regresses on barrel, V13 is inert-or-eyeballed, V14 untested, and the **full** V16 foundation (`v16.5`, board-range filter included) fails broad_mix — but the foundation **minus** that filter (`v16_noranges`) is the validated promote. **Recommended submission: `v16_noranges`** (highest worst-field floor among passing candidates is a wash with nr_evsize; noranges wins on the lower-risk diff + its polar3bet edge). V7-M2 (fix) is the fallback.
