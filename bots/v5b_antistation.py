"""
================================================================================
V5b — anti-calling-station exploit module (built on frozen v4.1 classifier).
Base: v4_1_classifier_retuned.py (UNMODIFIED). Adds ONLY anti-station logic.

WHAT CHANGES (only when the opponent we're facing is a detected station):
  1. Value bet THINNER  — bet good-but-not-nutted hands more often when checked
     to, and bet thin showdown hands that v4.1 would check;
  2. LARGER value sizing — strong made hands bet/raise bigger (stations pay off);
  3. FEWER / NO bluffs   — suppress the dry-board steals and thin probe-raises
     that v4.1 occasionally fires (stations don't fold to them);
  4. Call LESS marginally when the station suddenly raises/jams — a station
     showing aggression is uncharacteristically strong, so tighten our calling
     threshold against it.
Outside a confirmed station opponent, behavior is byte-for-byte identical to
v4.1. The whole module is gated by _ANTISTATION_ENABLED (set False to disable).

Triggers (any one, evaluated for the relevant opponent):
  * high-confidence implementation read top_impl == "calling_bot", OR
  * behavior read top_behav == "station" with confidence >= 0.65, OR
  * observed call-vs-bet very high: faced_bet >= 20 and
    call_vs_bet / faced_bet >= 0.65.

Explicitly NOT added: anti-perma logic, anti-folder logic, anti-Monte-Carlo
logic, or any general aggression change. Classification/collection logic is
...
# ===========================================================================
# V5b — anti-calling-station exploit module (the ONLY behavior change vs v4.1)
# ===========================================================================
# Capped, defensive, easy to disable. Reads existing per-opponent stats and
# LAST_READ only; never mutates classification state.

_ANTISTATION_ENABLED = True        # flip to False to fully disable V5b

# Sizing / frequency knobs (capped so we never spew).
_AS_THIN_VALUE_FLOOR = 0.46        # bet thin value down to this equity vs station
_AS_VALUE_BET_PROB = 0.92          # how often to value-bet good hands when checked to
_AS_STRONG_SIZE = 0.95             # strong-hand value size (pot fraction) vs station
_AS_THIN_SIZE = 0.55               # thin value size (pot fraction) vs station
_AS_RAISE_CALL_MARGIN = 0.10       # extra equity required to call a station's raise


def _opp_is_station(bot_id):
    """True if this specific opponent meets any station trigger:
      (a) high-confidence top_impl == 'calling_bot' (for THIS villain), OR
      (b) top_behav == 'station' with confidence >= 0.65 (for THIS villain), OR
      (c) observed call-vs-bet very high: faced_bet >= 20 and
          call_vs_bet/faced_bet >= 0.65.
    ...
    if st:
        ...
        if fb >= 20 and (st.get("call_vs_bet", 0) / fb) >= 0.65:   # (c)
            return True
    try:
        if LAST_READ.get("villain") == bot_id:
            if (LAST_READ.get("top_impl") == "calling_bot"
                    and LAST_READ.get("confidence", 0.0) >= 0.70):  # (a)
                return True
            if (LAST_READ.get("top_behav") == "station"
                    and LAST_READ.get("confidence", 0.0) >= 0.65):  # (b)
                return True
    except Exception:
        pass
    return False
...
def _current_opponent_id(state):
    """Best-effort id of the single relevant opponent: the current aggressor if
    we're facing a bet, else the main villain from LAST_READ, else the lone live
    opponent. Read-only; never raises."""
    me = state.get("seat_to_act")
    seat_to_id = {}
    live = []
    for p in state.get("players") or []:
        try:
            ...
            continue
    # aggressor we're facing (last non-blind raise/all_in not by us)
    last = None
    try:
        for a in state.get("action_log", []):
...
def _antistation_active(state):
    """True iff V5b is enabled AND the relevant opponent is a detected station."""
...
def _facing_opp_raise_this_street(state):
    """True if the opponent has raised/jammed on the CURRENT street (used to
    detect a station 'suddenly' showing aggression). Read-only."""
    me = state.get("seat_to_act")
    # current street is implied by community card count; we approximate 'this
    # street' as: there is an outstanding bet to us (owed>0) created by a
    # raise/all_in from the opponent in the log.
...
def _safetag(state):
    ...
    as_active = _antistation_active(state)   # V5b: False unless facing a station
    ...
    eq = equity_vs_random(hole, board, n_opp, rng=rng)
    if eq is not None:
        return _postflop_by_equity(state, eq, pos, can_check, board, rng, as_active)
    # Fallback: V1 heuristic buckets.
    return _postflop_by_heuristic(state, hole, board, pos, can_check, rng, as_active)


def _postflop_by_equity(state, eq, pos, can_check, board, rng, as_active=False):
    req = _required_equity(state)

    # ---- V5b anti-station overrides (only when as_active) --------------------
    # Capped, defensive: thinner value when checked to, bigger value sizing,
    # no bluffs, and a tighter calling threshold when the station raises/jams.
    if as_active:
        if can_check:
            # Value bet thinner & bigger; never bluff air.
            if eq >= 0.72:
            ...
            if eq >= _AS_THIN_VALUE_FLOOR:
                # thin value the station will pay off; v4.1 would just check here
                if rng.random() < 0.5:
                    return {"action": "raise", "amount": _raise_to(state, 0.5)}
                return {"action": "check"}
            # weak/air vs a station: check, NEVER bluff (suppress the steal)
            return {"action": "check"}
        else:
            # Facing a bet from a station. If the station has actually RAISED/
            # jammed (uncharacteristic), tighten our calling threshold; never
            # spew calls. Value-raise strong hands bigger.
            ...
            if eq >= 0.38:
                ...
                return {"action": "fold"}
            return {"action": "fold"}
    # ---- end anti-station overrides; below is v4.1-identical -----------------
...
def _postflop_by_heuristic(state, hole, board, pos, can_check, rng, as_active=False):
    ...
    if as_active:
        ...
        if bucket == "medium":
            if can_check:
                # bet thin value into a station instead of probing as a bluff
                if rng.random() < 0.6:
                    return {"action": "raise", "amount": _raise_to(state, _AS_THIN_SIZE)}
                return {"action": "check"}
            tighten = _AS_RAISE_CALL_MARGIN if _facing_opp_raise_this_street(state) else 0.0
            return {"action": "call"} if req <= 0.33 - tighten else {"action": "fold"}
        if drawy:
            # draws: just take the price; no semibluff vs a station (won't fold)
            if can_check:
                return {"action": "check"}
            return {"action": "call"} if req <= 0.30 else {"action": "fold"}
        # weak/air vs a station: never bluff
