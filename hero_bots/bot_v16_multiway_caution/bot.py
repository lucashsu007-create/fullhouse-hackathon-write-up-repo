...
BOT_NAME = "v16_arch_multiway_caution"
BOT_AVATAR = "robot_1"
...
_V16_FLAGS = {
    ...
    "action_filter": False,
    "blocker_bluff": False,
    "multiway_caution": True,
    "river_underbluff_fold": False,
}
...
# Fires ONLY at a frequency no sound strategy occupies; corrects an INPUT (the
# villain's range bucket), never overrides policy. Zero-cost when it never fires.
_ARCH_ALLIN_MIN, _ARCH_ALLIN_RATE = 5, 0.90     # perma all-inner
_ARCH_MANIAC_MINA, _ARCH_MANIAC_RATE = 10, 0.85 # perma raiser / maniac

def _arch_is_maniac(stats):
    """True iff this villain shoves ~always OR raises ~always — i.e. their
    aggression is ~random, not strength. Conservative sample gates; no sound
    player reaches an 85-90%+ raise/shove rate, so false positives are ~nil."""
    if not stats:
        return False
    a = stats.get("actions", 0)
...
def _lone_villain_stats(state):
    """Stats for the single live opponent (heads-up reads only)."""
...
def _range_and_filter_for_villain(state, villain_seat, villain_bot_id):
    ...
    # [archetype Tier-1, HARD] perma all-inner / perma raiser -> their range is
    # ~random, NOT strong. Override to UNKNOWN (= any-2 random in the equity
    # engine) so we call/continue with true favourites instead of folding to a
    # coin-flip merchant that the action-implies-strength heuristic miscodes as
    # NUTTED. 100% confidence: only past an extreme cliff no sound play reaches.
...
def _postflop_by_equity(state, eq, pos, can_check, board, rng):
    req = _required_equity(state)
    ev = _flag("ev_call")
    n_opp = _n_live_opponents(state) if (ev or _flag("multiway_caution")
                                         or _flag("river_underbluff_fold")) else 0
    hole = state.get("your_cards", []) or []
    # [multiway_caution] >=2 live opponents: demand more equity to call thin and
    # to value-bet/raise (input bumps; band logic below untouched). Off => identical.
    if _flag("multiway_caution") and (n_opp or 0) >= 2:
        req += 0.03
        if not can_check and eq < 0.82:
            eq = min(eq, 0.71)
        elif can_check and eq < 0.82:
            eq = min(eq, 0.54)
    # [river_underbluff_fold] fold more to BIG river bets/jams from LOW-aggression
    # villains (value-heavy aggression in an underbluffing field). Gated on a
    # confirmed low raise_rate so it never fires vs a maniac. Off => identical.
