...
BOT_NAME = "v16_squeeze_C_metacap_blockstraight"
BOT_AVATAR = "robot_1"
...
_V16_FLAGS = {
    ...
    "action_filter": False,
    "blocker_bluff": False,
    "river_underbluff_fold": True,   # fold more to big river bets/jams from low-aggr villains
    "nutted_value_sizing": True,      # test-only: full house+ / nut flush / safe set low-SPR sizing
    "premium_squeeze": True,          # test-only: AA/KK/QQ/AKs squeeze over open + caller(s)
    "tiny_size_tweak": False,         # ablation: disabled, isolates squeeze only
}
...
# ===========================================================================
# Test-only v16 + conservative preflop squeeze / sizing tweak
# ===========================================================================

_V16_SQUEEZE_HANDS = {"AA", "KK", "QQ", "AKs"}  # AA/KK/QQ/AKs squeeze only, no tiny sizing tweak
_V16_PREMIUM_3BET_HANDS = {"AA", "KK", "QQ", "AKs", "AKo"}


def _v16_cap_raise_to(state, target):
    """Snap a desired raise-to amount to the engine's legal band."""
    try:
        min_to = int(state.get("min_raise_to", 0) or 0)
        cap = int((state.get("your_stack", 0) or 0) +
...
def _v16_callers_after_first_raise(state):
    """Count preflop callers after the first open and before any second raise."""
    seen_open = False
    callers = 0
    try:
...
def _v16_premium_squeeze_action(state, hand, history, facing_jam):
    """Premium-only squeeze over open + caller(s). No bluffs, no jam widening."""
    if not _flag("premium_squeeze"):
        return None
    if facing_jam or hand not in _V16_SQUEEZE_HANDS:
        return None
    if history.get("raises") != 1 or history.get("first_raiser_was_us"):
        return None
    callers = _v16_callers_after_first_raise(state)
    if callers < 1:
        return None
    if state.get("min_raise_to", 0) <= 0:
        return None
    opener_to = int(state.get("current_bet", 0) or 0)
    pot = int(state.get("pot", 0) or 0)
    # Conservative squeeze: roughly 4x open + 1x per caller, but at least a
    # pot-pressure raise. Total amount, not additional chips.
    target = max(int(opener_to * (4.0 + callers)), opener_to + int(0.9 * pot))
    return _v16_cap_raise_to(state, target)


def _v16_open_size_tweak(pos, size_bb):
    if not _flag("tiny_size_tweak"):
        return size_bb
    # Slightly increase late-position steal/value pressure without widening.
    if pos in ("CO", "BTN", "SB"):
        return size_bb + 0.2
    return size_bb


def _v16_premium_3bet_multiplier(hand, multiplier):
    if not _flag("tiny_size_tweak"):
        return multiplier
    if hand in _V16_PREMIUM_3BET_HANDS:
...
def _preflop_v6(state):
    ...
    squeeze_action = _v16_premium_squeeze_action(state, hand, history, facing_jam)
    if squeeze_action is not None:
        return squeeze_action
...
def _open_branch(state, hand, pos, rng):
    ...
    if choice == "raise":
        ...
        size_bb = _v16_open_size_tweak(pos, size_bb)
...
def _hu_btn_open(state, hand, rng):
    ...
    if hand in _HU_BTN_NEVER_OPEN:
        if rng.random() < 0.10:
            return {"action": "raise", "amount": _bb_to_chips(state, _v16_open_size_tweak("BTN", 2.5))}
        if state.get("can_check", False):
            return {"action": "check"}
        return {"action": "fold"}
    return {"action": "raise", "amount": _bb_to_chips(state, _v16_open_size_tweak("BTN", 2.5))}
...
def _defense_branch(state, hand, our_pos, history, rng):
    ...
    if choice == "3bet":
        ...
        multiplier = _v16_premium_3bet_multiplier(hand, multiplier)
...
def _postflop_pos_score(state):
    ...
    Test-only correction: preflop chart logic remains unchanged; postflop
    probes/value bets stop treating a high absolute seat number as always late.
    """
    try:
        me = state.get("seat_to_act")
        players = state.get("players") or []
...
# ===========================================================================
# Test-only v16 + nut-region value sizing
# ===========================================================================

def _v16nv_rank_counts(cards):
    d = {}
    for c in cards or []:
        try:
            r = c[0]
...
def _v16nv_straight_window_hits(cards):
    vals = set()
    for c in cards or []:
        try:
...
def _v16nv_board_profile(board):
    b = board or []
    rc = _v16nv_rank_counts(b)
    sc = _v16nv_suit_counts(b)
    ...
    straight_hits = _v16nv_straight_window_hits(b)
...
def _v16nv_hero_flush_rank(hole, board):
    cards = (hole or []) + (board or [])
    sc = _v16nv_suit_counts(cards)
...
def _v16nv_bucket(hole, board):
    try:
        ...
        if len(hole) != 2 or len(board) < 3:
            return "unknown"
        rc_all = _v16nv_rank_counts(cards)
        rc_board = _v16nv_rank_counts(board)
        counts = sorted(rc_all.values(), reverse=True)
        if counts and counts[0] >= 4:
            return "full_house_plus"
        if len(counts) >= 2 and counts[0] >= 3 and counts[1] >= 2:
            return "full_house_plus"
        fr = _v16nv_hero_flush_rank(hole, board)
        if fr is not None:
            return "nut_flush" if fr >= 13 else "low_flush"
        if _v16nv_straight_window_hits(cards) >= 5:
            return "straight"
...
def _v16_nutvalue_sizing(action, state, board):
    """Tiny test patch: only increase sizing for nut-region hands.

    No fake-strength folds, no c-bet pressure, no multiway caution, no EV-call.
    """
    if not _flag("nutted_value_sizing") or not isinstance(action, dict):
        return action
    try:
        if state.get("street") == "preflop":
            return action
        hole = state.get("your_cards") or []
        bucket = _v16nv_bucket(hole, board or [])
        prof = _v16nv_board_profile(board or [])
...
def _safetag(state):
    ...
    if eq is not None:
        ...
        return _v16_nutvalue_sizing(raw, state, board)
...
# ===========================================================================
# Test-only capped-line meta counters
# ===========================================================================

def _v16mc_bb_amount(state):
    try:
        for a in state.get("action_log") or []:
            if isinstance(a, dict) and a.get("action") == "big_blind":
                amt = a.get("amount")
...
def _v16mc_can_raise(state):
    try:
        min_to = state.get("min_raise_to", 0) or 0
        cap = (state.get("your_stack", 0) or 0) + (state.get("your_bet_this_street", 0) or 0)
        return min_to > 0 and cap > min_to
    except Exception:
        return False


def _v16mc_raise_total(state, target):
    try:
        min_to = state.get("min_raise_to", 0) or 0
        cap = (state.get("your_stack", 0) or 0) + (state.get("your_bet_this_street", 0) or 0)
        if min_to <= 0 or cap <= 0:
            return None
        amt = int(max(target, min_to))
        return min(amt, cap)
    except Exception:
        return None


def _v16mc_preflop_limper_count(state):
    """Count preflop calls before any voluntary raise. Blinds do not count."""
    try:
        limps = 0
        for rec in _reconstruct_hand(state.get("action_log") or []):
            if rec.get("street_idx") != 0:
...
def _v16mc_preflop_limp_punish(state):
    try:
        if state.get("street") != "preflop" or not _v16mc_can_raise(state):
            return None
        hand = _canonical(state.get("your_cards") or [])
        if hand not in {"AA", "KK", "QQ", "AKs"}:
            return None
        limps, has_raise = _v16mc_preflop_limper_count(state)
        if has_raise or limps <= 0:
            return None
        bb = _v16mc_bb_amount(state)
        # Tight exploit of capped limp ranges only: 5bb + 1bb per limper.
        amt = _v16mc_raise_total(state, (5 + limps) * bb)
        if amt is None:
            return None
...
def _v16mc_preflop_minraise_punish(state):
    try:
        if state.get("street") != "preflop" or not _v16mc_can_raise(state):
            return None
        hand = _canonical(state.get("your_cards") or [])
        if hand not in {"AA", "KK", "QQ", "AKs"}:
            return None
        hist = _preflop_history(state)
        if hist.get("raises", 0) != 1 or hist.get("last_raiser_was_us"):
            return None
        bb = _v16mc_bb_amount(state)
        cur = state.get("current_bet", 0) or 0
        if cur <= 0 or cur > 2.2 * bb:
            return None
        # Punish min-open with a standard large value 3-bet, no jams.
        amt = _v16mc_raise_total(state, 4.8 * cur)
        if amt is None:
            return None
        return {"action": "raise", "amount": amt}
    except Exception:
...
def _v16mc_board_max_rank(board):
    vals = [_RANK_VAL.get(c[0], 0) for c in (board or []) if c]
    return max(vals) if vals else 0


def _v16mc_clean_overpair(hole, board):
    try:
        if len(hole or []) != 2 or len(board or []) < 3:
            return False
        if hole[0][0] != hole[1][0]:
            return False
        pair_val = _RANK_VAL.get(hole[0][0], 0)
        if pair_val <= _v16mc_board_max_rank(board):
            return False
        prof = _v16nv_board_profile(board or [])
        return not prof.get("paired") and not prof.get("four_straight") and not prof.get("four_flush")
    except Exception:
        return False


def _v16mc_real_two_pair(hole, board):
    try:
        if len(hole or []) != 2 or len(board or []) < 3:
            return False
        if hole[0][0] == hole[1][0]:
        ...
        if hole[0][0] not in board_ranks or hole[1][0] not in board_ranks:
            return False
        prof = _v16nv_board_profile(board or [])
        return not prof.get("paired") and prof.get("danger", 0) <= 2
    except Exception:
        return False


def _v16mc_value_bucket(hole, board):
    try:
        b = _v16nv_bucket(hole or [], board or [])
        if b in {"full_house_plus", "nut_flush", "set"}:
            return b
        if _v16mc_clean_overpair(hole or [], board or []):
            return "clean_overpair"
        if _v16mc_real_two_pair(hole or [], board or []):
...
def _v16mc_tiny_donk_punish(state):
    try:
        street = state.get("street")
        if street not in {"flop", "turn"} or not _v16mc_can_raise(state):
            return None
        owed = state.get("amount_owed", 0) or 0
        pot = max(1, state.get("pot", 0) or 1)
        if owed <= 0 or owed > 0.20 * pot:
            return None
        hole = state.get("your_cards") or []
        board = state.get("community_cards") or []
        vb = _v16mc_value_bucket(hole, board)
        if vb not in {"clean_overpair", "real_two_pair", "set", "nut_flush", "full_house_plus"}:
            return None
        prof = _v16nv_board_profile(board or [])
        if vb == "set" and prof.get("danger", 0) >= 3:
            return None
        # Raise tiny lead to a real price. Amount is total street bet.
        amt = _raise_to(state, 0.70)
        return {"action": "raise", "amount": amt}
    except Exception:
        return None


def _v16mc_river_block_punish(state):
    try:
        if state.get("street") != "river" or not _v16mc_can_raise(state):
            return None
        owed = state.get("amount_owed", 0) or 0
        pot = max(1, state.get("pot", 0) or 1)
        if owed <= 0 or owed > 0.25 * pot:
            return None
        hole = state.get("your_cards") or []
        board = state.get("community_cards") or []
        b = _v16nv_bucket(hole, board)
        if b not in {"full_house_plus", "nut_flush", "straight"}:
            return None
        if b == "straight":
            prof = _v16nv_board_profile(board or [])
            # Only raise safe board-blocker straights: no paired board and no live flush texture.
            if prof.get("paired") or prof.get("three_flush") or prof.get("four_flush"):
                return None
        spr = _v16nv_spr(state)
        if spr <= 2.4:
            return {"action": "all_in"}
...
def _v16_metacap(action, state):
    """Very narrow capped-line punishers only.

    No defensive folds, no opponent-type reads, no extra bluffs, no range widening.
    """
    try:
        # Preflop capped lines first. These override calls/normal opens only with premiums.
        if state.get("street") == "preflop":
            for fn in (_v16mc_preflop_limp_punish, _v16mc_preflop_minraise_punish):
                cand = fn(state)
                if cand is not None:
                    return cand
            return action
        # Postflop under-sized aggression punishers, value-only.
        for fn in (_v16mc_river_block_punish, _v16mc_tiny_donk_punish):
            cand = fn(state)
            if cand is not None:
                return cand
    except Exception:
...
def _sanitize(action, state):
    ...
    if a == "raise":
        try:
            ...
            amt = state.get("min_raise_to", 0) or 0
        min_to = state.get("min_raise_to", 0) or 0
        cap = (state.get("your_stack", 0) or 0) + (state.get("your_bet_this_street", 0) or 0)
        if min_to <= 0 or cap <= 0 or cap < min_to:
            if can_check:
                return {"action": "check"}
            return {"action": "call"} if (state.get("amount_owed", 0) or 0) > 0 else {"action": "fold"}
        amt = max(amt, min_to)
    ...
    try:
        ...
        if stats_changed:
            try:
                ...
                pass
        base_action = _safetag(game_state)
        base_action = _v16_metacap(base_action, game_state)
        return _sanitize(base_action, game_state)
    except Exception:
