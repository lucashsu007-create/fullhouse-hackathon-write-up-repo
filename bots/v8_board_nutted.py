"""
================================================================================
v8 — board-conditioned NUTTED range on top of v7.

WHAT CHANGED vs v7 (postflop equity only; preflop charts and everything else
byte-identical):

  * NUTTED range is now board-aware. v7 used the static string "JJ+, AKs, AKo"
    on every board, which is catastrophically wrong on coordinated boards.
    Example: AA facing a rock-raise on T98hh — v7 computed 62% equity (vs the
    JJ+/AK range it imagined), but vs the REAL rock range (a flush or set)
    AA only has 6% equity. v8 instead enumerates combos that achieve a
    threshold hand strength on the actual board:
        - 4-to-flush / monotone boards : require Flush or better
        - 4-to-straight boards         : require Straight or better
        - Paired boards (one pair on board) : require Trips or better
        - All other boards             : require Two Pair or better
    On dry boards where the strict filter gives too few combos (<15) we
    expand to also include hands that interact with the board (overpairs,
    top pair top kicker) so the MC has enough sample.

WHAT'S UNCHANGED FROM v7:
  * All chart-preflop logic
  * Postflop decision tree
  * V3 stats, V4 classifier
  * Spot RNG / paired A/B determinism
  * The WIDE / OPEN / THREEBET / UNKNOWN range buckets (only NUTTED is
    board-conditioned in v8; the others use preflop range strings as before
    and rely on eval7 MC to handle which combos connect to the board)
  * The passive-rocket override trigger (PLAYER_STATS-confirmed passive
    villain who raised this hand). v8 changes what NUTTED MEANS on this
    board, not when it triggers.

Performance: building the board-conditioned pool is ~1ms (enumeration of
~990 combos + eval7.evaluate each). Equity MC itself unchanged.
    ...
    for rs in opp_range_strs:
        if rs == "UNKNOWN":
            ...
            if not nutted_pool_cache:
                # Pool ended up empty (e.g. board allows no two-pair+). Fall
                # back to the static NUTTED range string so we never silently
                # crash. This branch should be very rare in practice.
