...
BOT_NAME = "v16_squeeze_E_position_aware"
BOT_AVATAR = "robot_1"
...
_V16_SQUEEZE_HANDS = {"AA", "KK", "QQ", "AKs", "AKo"}  # max universe, filtered by position below
...
def _v16_premium_squeeze_action(state, hand, history, facing_jam):
    """Position-aware premium squeeze ablation.

    No bluffs, no jam widening, no unrelated sizing tweaks.
    Because action logs do not always expose a clean opener position, this uses
    hero position as a robust proxy for IP/OOP risk:
      - early hero positions stay tight: AA/KK/QQ
      - CO/BTN/SB can include AKs
      - BTN/BB can include AKo
    """
    ...
    pos = _hero_position_name(state) or "UNK"
    if pos in ("UTG", "HJ"):
        allowed = {"AA", "KK", "QQ"}
        mult = 4.5
    elif pos in ("CO", "SB"):
        allowed = {"AA", "KK", "QQ", "AKs"}
        mult = 4.5
    else:  # BTN / BB / fallback, usually more playable or closing action
        allowed = {"AA", "KK", "QQ", "AKs", "AKo"}
        mult = 3.8
    if hand not in allowed:
        return None

    opener_to = int(state.get("current_bet", 0) or 0)
    pot = int(state.get("pot", 0) or 0)
    target = max(int(opener_to * (mult + callers)), opener_to + int(0.9 * pot))
