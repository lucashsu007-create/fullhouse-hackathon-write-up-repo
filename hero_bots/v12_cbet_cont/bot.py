...
def _safetag(state):
    ...
    eq = _hero_equity(hole, board, state, n_opp, rng)
    if eq is not None:
        return _postflop_dispatch(state, eq, pos, can_check, board, rng)
...
# ===========================================================================
# v12 prototypes -- (1) bread-and-butter c-bet bluffs  (2) continuous postflop
# Both OFF by default; each prototype file flips the flags below. The bluff is
# triggered by BOARD TEXTURE + our own line (not by an opponent read); the only
# opponent signal is a ONE-SIDED off-switch -- suppress the bluff vs a confirmed
# check-raiser (bet_punisher_read on raise_vs_bet). So it cannot misfire-bleed
# the way a read-triggered bluff does: it can only ever bluff LESS, never more.
# ===========================================================================

USE_CONTINUOUS = True      # True -> smooth postflop policy instead of equity bands
USE_CBET       = True      # True -> add hard-coded flop c-bet bluffs (PFR, heads-up)

CBET_SIZE          = 0.50  # c-bet pot fraction
CBET_BLUFF_CAP     = 0.55  # only c-bet-bluff hands weaker than this (give-up zone)
CBET_FREQ_DRY      = 0.60  # range-bet frequency on a dry board (air or draw)
CBET_FREQ_WET_DRAW = 0.50  # semi-bluff frequency on a wet board (draws only)
CBET_RAISE_CAP     = 0.15  # raise_vs_bet shrunk-mean above this -> check-raiser -> OFF

CONT_CALL_FLOOR    = 0.42  # never call below this facing a bet (v7_m2 floor)
CONT_CUSHION       = 0.03  # uniform domination cushion; monotonic, fixes the 0.55
                           # seam (where eq 0.54 called but 0.56 folded)


def bet_punisher_read(stats, raise_cap=0.15, prior_m=0.10, prior_k=8.0, min_n=20):
    """True if the villain raises bets often enough to be a check-raiser/trap.
    Shrunk toward a low population rate so thin/unknown villains are NOT flagged."""
    if not stats:
        return False
    faced = stats.get("faced_bet", 0)
...
def _postflop_continuous(state, eq, pos, can_check, board, rng):
    """Continuous analogue of _postflop_by_equity: same shape (value-bet ahead,
    pot-odds call, give up weak) but smooth in equity, with a MONOTONIC facing-
    a-bet call rule. Anchored to reproduce v7_m2 near the band centers; differs
    mainly by smoothing the medium bet frequency/size and a small uniform call
    cushion. This is a genuine policy change (does NOT preserve v7_m2 action-for-
    action) -- which is exactly what the A/B is meant to measure."""
    req = _required_equity(state)
    if can_check:
        bet_freq = _clamp01((eq - 0.50) / 0.25)            # 0 @0.50 -> 1 @0.75
        if eq < 0.72:                                      # v7_m2 bets medium only IP
            bet_freq *= (0.55 + 0.45 * _clamp01(pos))
        size = 0.50 + 0.40 * _clamp01((eq - 0.55) / 0.30)  # 0.5 -> 0.9 pot
        if rng.random() < bet_freq:
            return {"action": "raise", "amount": _raise_to(state, size)}
        if eq < 0.42 and pos > 0.6 and _board_is_dry(board) and rng.random() < 0.18:
            return {"action": "raise", "amount": _raise_to(state, 0.5)}
        return {"action": "check"}
    if eq >= 0.72:                                         # facing a bet, strong
        if eq >= 0.85 and rng.random() < 0.6:
            return {"action": "raise", "amount": _raise_to(state, 0.9)}
        return {"action": "call"}
    if eq >= max(CONT_CALL_FLOOR, req + CONT_CUSHION):     # monotonic call
        return {"action": "call"}
    return {"action": "fold"}


def _cbet_override(state, base, eq, can_check, board):
    """If the active policy would CHECK as the preflop raiser on the flop, heads-
    up, with a weak hand, convert it to a c-bet bluff at a board-dependent
    frequency: range-bet dry boards, semi-bluff (draws only) on wet boards, give
    up with air on wet boards. OFF vs a confirmed check-raiser. Only ever turns a
    check INTO a bet -- never touches a value bet, a call, or a fold. Uses a
    salted RNG so when it declines the base policy's roll is untouched."""
    if not USE_CBET or base.get("action") != "check":
        return base
    if state.get("street") != "flop" or eq >= CBET_BLUFF_CAP:
        return base
    villains = _live_villains(state)
    if len(villains) != 1:
        return base                                        # heads-up only
    if not _preflop_history(state).get("first_raiser_was_us"):
        return base                                        # must be the preflop raiser
    if bet_punisher_read(PLAYER_STATS.get(villains[0][1]), raise_cap=CBET_RAISE_CAP):
        return base                                        # off vs check-raiser
    hole = state.get("your_cards", []) or []
    _b, fd, oesd = _postflop_strength(hole, board)
    if _board_is_dry(board):
        freq = CBET_FREQ_DRY                               # range-bet
    elif fd or oesd:
        freq = CBET_FREQ_WET_DRAW                          # semi-bluff only
    else:
        return base                                        # wet board + air -> give up
    if random.Random(_spot_seed(state) ^ 0xC4E7B10F).random() < freq:
        return {"action": "raise", "amount": _raise_to(state, CBET_SIZE)}
    return base


def _postflop_dispatch(state, eq, pos, can_check, board, rng):
    """Single postflop entry: pick the base policy (discrete v7_m2 bands or the
    continuous analogue), then optionally apply the c-bet bluff override."""
    if USE_CONTINUOUS:
        base = _postflop_continuous(state, eq, pos, can_check, board, rng)
    else:
        base = _postflop_by_equity(state, eq, pos, can_check, board, rng)
    return _cbet_override(state, base, eq, can_check, board)
